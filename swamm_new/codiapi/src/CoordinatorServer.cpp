...
int CCoordinatorServer::RegisterCoordinator(COORDINATOR **pReturn, CODIDEVICE *pDevice)
{
	...
	//
	//fprintf(stderr,"DELETEME: register coordinator start\n");
	m_codiLocker.Lock();
	codi = FindCoordinatorByName(pDevice->szDevice);
	if (codi == NULL)
	{
	//fprintf(stderr,"DELETEME: register coordinator: not found\n");
		codi = AddCoordinator(pDevice);
		if (codi != NULL)
			 nError = CODIERR_NOERROR;
		else nError = CODIERR_MEMORY_ERROR;
		*pReturn = codi;
	//fprintf(stderr,"DELETEME: register coordinator: register %d\n", nError);
	}
	else nError = CODIERR_DEVICE_INUSE;
	m_codiLocker.Unlock();
	//fprintf(stderr,"DELETEME: register coordinator: result %d\n", nError);
	return nError;
}
...
int CCoordinatorServer::StartupCoordinator(COORDINATOR *codi)
{
	...
	//fprintf(stderr,"DELETEME: startup coordinator\n");
	m_codiLocker.Lock();
...
int	CCoordinatorServer::BootloadCoordinator(COORDINATOR *codi, int nType)
{
	...
	nError = SetCoordinatorProperty(codi, CODI_CMD_BOOTLOAD, (BYTE *)&payload,
...
int CCoordinatorServer::StartCoordinator(COORDINATOR *codi)
{
	...
	//fprintf(stderr,"DELETEME: Start coordinator\n");
	if (codi->bStarted)
		return CODIERR_ALREADY_STARTED;

	//fprintf(stderr,"DELETEME: Start coordinator: type check\n");
	switch(codi->device.nType) {
	  case CODI_COMTYPE_RS232 :
		   pInterface = new CSerialInterface();
	//fprintf(stderr,"DELETEME: Start coordinator: RS232\n");
		   break;
	  case CODI_COMTYPE_VIRTUAL :
		   pInterface = new CVirtualInterface();
	//fprintf(stderr,"DELETEME: Start coordinator: Virtual\n");
		   break;
	  case CODI_COMTYPE_SPI :
	  default :
	//fprintf(stderr,"DELETEME: Start coordinator: Other\n");
		   break;
	}

	//fprintf(stderr,"DELETEME: Start coordinator: pInterface %p\n", pInterface);
	if (pInterface == NULL)
...
COORDINATOR *CCoordinatorServer::FindCoordinatorByHandle(COORDINATOR *pCoordinator)
{
	...
	//fprintf(stderr,"DELETEME: find coordinator start\n");
	codi = m_codiList.GetFirst(pos);
	while(codi)
	{
	//fprintf(stderr,"DELETEME: find coordinator get first\n");
		if (codi == pCoordinator)
			break;
		codi = m_codiList.GetNext(pos);
	//fprintf(stderr,"DELETEME: find coordinator get nest\n");
	}	
	//fprintf(stderr,"DELETEME: find coordinator end\n");
	return codi;
}
