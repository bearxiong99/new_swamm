...
int CDbGroup::db_Table_Check(const char* dbfilename)
{
	try
	{
		CppSQLite3DB db;
		...
		db.execDML("CREATE TABLE IF NOT EXISTS member(EUI64ID CHAR(8) PRIMARY KEY, Model TEXT, MeterSerial TEXT, HWver CHAR(10), SWver CHAR(10), SensorState CHAR(10), NetworkPathInfo TEXT);");
		db.execDML("CREATE TABLE IF NOT EXISTS groupmember(GroupKey INTEGER, EUI64ID CHAR(8), FOREIGN KEY(GroupKey) REFERENCES groups(GroupKey), FOREIGN KEY(EUI64ID) REFERENCES member(EUI64ID));");
		db.execDML("CREATE TABLE IF NOT EXISTS command(CommandKey INTEGER PRIMARY KEY, GroupKey INTEGER, EUI64ID CHAR(8), GroupName TEXT, CommandState INTEGER, Command TEXT, CommandCreateTime TEXT, CommandExecuteTime TEXT, ResultErrorCode INTEGER, ParamPathInfo TEXT, FOREIGN KEY(GroupKey) REFERENCES groups(GroupKey), FOREIGN KEY(EUI64ID) REFERENCES member(EUI64ID));");
		db.close();
	}
...
//////////////////////////////////////////////////////////////////////////
int CDbGroup::db_Group_Create(const char* dbfilename)
{
	try
	{
		CppSQLite3DB db;
...
//////////////////////////////////////////////////////////////////////////
int CDbGroup::db_Member_Create(const char* dbfilename)
{
	sqlite3* db = NULL;
	...
	if(sqlite3_exec(db, "CREATE TABLE IF NOT EXISTS member(EUI64ID CHAR(8) PRIMARY KEY, Model TEXT, MeterSerial TEXT, HWver CHAR(10), SWver CHAR(10), SensorState CHAR(10), NetworkPathInfo TEXT)", NULL, NULL, NULL) != SQLITE_OK)
	{
		XDEBUG("SQLite create failed: %s\r\n", sqlite3_errmsg(db));
		return sqlite3_errcode(db);
	}
...
//  (GroupKey INTEGER - Foreign Key, EUI64ID CHAR(8) - Foreign Key
//////////////////////////////////////////////////////////////////////////
int CDbGroup::db_GroupMemberRelation_Create(const char* dbfilename)
{
	sqlite3* db = NULL;
	...
	if(sqlite3_exec(db, "CREATE TABLE IF NOT EXISTS groupmemberrelation(GroupKey INTEGER, EUI64ID CHAR(8), FOREIGN KEY(GroupKey) REFERENCES groups(GroupKey), FOREIGN KEY(EUI64ID) REFERENCES member(EUI64ID)", NULL, NULL, NULL) != SQLITE_OK)
	{
		XDEBUG("SQLite create failed: %s\r\n", sqlite3_errmsg(db));
		return sqlite3_errcode(db);
	}
...
//////////////////////////////////////////////////////////////////////////
int CDbGroup::GroupAdd(const char* dbfilename, char* szGroupName)
{
	int nGroupKey = -1;
	try
	{
...
int CDbGroup::GroupAddMember(const char* dbfilename, int nGroupKey, char* szMemberId)
{
	try
	{
		CppSQLite3DB db;
...
int CDbGroup::GroupDelete(const char* dbfilename, int nGroupKey)
{
	try
	{
		CppSQLite3DB db;
...
int CDbGroup::GroupDeleteMember(const char* dbfilename, int nGroupKey, char* szMemberId)
{
	try
	{
		CppSQLite3DB db;
...
int CDbGroup::GroupInfo(const char* dbfilename, GROUPINFO *Group)
{
	EUI64				memberID;
	char				szID[8] = {0,};
	char*				pMember = NULL;
	...
	try
	{
        ...
        */
		...
		Group->pGroupName = (char *)MALLOC(64*Group->nTotalCnt);
		Group->nMemberCnt = (int *)MALLOC(sizeof(int)*Group->nTotalCnt);
		Group->pMemberID = (EUI64 *)MALLOC(sizeof(EUI64)*Group->nTotalCnt);
		pMember = (char *)MALLOC(8*Group->nTotalCnt);
		memset(Group->pGroupName, 0, 64*Group->nTotalCnt);
		memset(pMember, 0, 8*Group->nTotalCnt);
		...
		for ( int row = 0; row < t.numRows(); row++)
		{
			...
			for (int fld=0; fld<t.numFields(); fld++)
			{
				if( !t.fieldIsNull(fld))
				{
					...
					case 1:
						...
						memcpy( &(Group->pGroupName[row*64]), t.fieldValue(fld), strlen( t.fieldValue(fld) ));
						break;
					//	MemberID
					case 2:
			...
			// XDEBUG("GroupName: %s\r\n", &(Group->pGroupName[row*64]));
			if(Group->nMemberCnt[row] != 0)
			{
				memcpy( &pMember[row*8], szID, sizeof(szID));
				// XDEBUG("pMember: %s\r\n", pMember);
				nMemCnt++;
...
int CDbGroup::GroupCommand(const char* dbfilename, int nGroupKey, vector<char*> vbufMember, MIBValue* pValue, int cnt)
{
...
int CDbGroup::GroupShowCommand(const char* dbfilename, int nGroupKey, GROUPCOMMANDINFO *pGroupCommandinfo)
{
	try
	{
		CppSQLite3DB db;
...
int CDbGroup::GroupTableDelete(const char* dbfilename, char* szTablekind)
{
	try
	{
		CppSQLite3DB db;
...
int CDbGroup::JoinTableShow(const char* dbfilename)
{
	try
	{
		CppSQLite3DB db;
...
int CDbGroup::JoinTableAdd(const char* dbfilename, char* szID)
{
	try
	{
		CppSQLite3DB db;
...
int CDbGroup::JoinTableDelete(const char* dbfilename, char* szID)
{
	try
	{
		CppSQLite3DB db;
...
int CDbGroup::JoinTableDelete(const char* dbfilename)
{
	try
	{
		CppSQLite3DB db;
...
int CDbGroup::JoinTableFileAdd(const char* dbfilename, char* szfilename)
{
	try
	{
		CppSQLite3DB db;
		...
		if(fp != NULL)
		{
			...
			while(fgets(pszBuffer, sizeof(pszBuffer)-1, fp) > 0)
			{
				if(strcmp(pszBuffer, "\n") == 0 || strcmp(pszBuffer, "\0") == 0)
					break;

				string buff(pszBuffer);
				XDEBUG("Get Line: %s\r\n", buff.c_str());
		...
		for(unsigned int i = 0; i < bufEUI64ID.size(); i++)
		{
			bufSQL.clear();
			XDEBUG("bufEUI64ID: %s\r\n", bufEUI64ID[i].c_str() );
			bufSQL.format("INSERT INTO jointable(EUI64ID, jointry) VALUES(%Q, %d);", bufEUI64ID[i].c_str(), 0);
			db.execDML(bufSQL);
		}
