...
BOOL CoordinatorPermitControl(BYTE permit_time)
{
	...
	nError = codiSetProperty(GetCoordinator(), CODI_CMD_PERMIT, (BYTE *)&permit, sizeof(CODI_PERMIT_PAYLOAD), 3000);
...
void codiOnDeviceJoin(HANDLE codi, EUI64 *id, WORD shortid, WORD parentid, BYTE status)
{
    ...
    // Issue #993 sensorJoinEvent가 발생하도록 코드 수정
	sensorJoinEvent(id);
...
{
    /** 스웨덴향 집중기는 Push Metering을 지원하지 않는다 */
}
