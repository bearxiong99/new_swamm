...
int	COndemander::CIDWrite(HANDLE codi, EUI64 *id, BYTE *value, int valuelen, BYTE *reply, int *replylen, BOOL bIsOpen, BOOL bIsConnect, BOOL bKeepConnect)
{
#if 0
    // REWRITER : TODO 반드시 다시 코딩해야 한다
	HANDLE		endi;
	char		szGUID[17];
    BYTE        szCommand[100];
    BYTE        reply2[100];
    ...
    BOOL        isOpen = bIsOpen;
    BOOL        isConnect = bIsConnect;
    WORD        kcrc, kcrcendian; 
    int         desuf_len=0;
	eui64toa(id, szGUID);
	XDEBUG("ONDEMAND: CID Write (ID=%s, Value=%s)\r\n", szGUID, value);
	if (codiGetState(codi) != CODISTATE_NORMAL)
	{
		XDEBUG("ONDEMAND: Coordinator not ready. (codiState=%d)\r\n", codiGetState(codi));
		return IF4ERR_NOT_READY;
        ...
        memset(szCommand, '\0', 100);
        szCommand[0]=0x80;
        szCommand[1]=0x3f;
        memcpy(szCommand+2, value, valuelen);

        kcrc = m_pKamstrupParser->KamCrcCalc(szCommand+1, valuelen+1);
        m_pKamstrupParser->ConvEndian(&kcrcendian, kcrc);
        memcpy(szCommand+2+valuelen, &kcrcendian, 2);
        szCommand[4+valuelen]=0x0d;
	...
	for(nRetry=0; nRetry<nCount; nRetry++)
	{
		...
		if (nError == CODIERR_NOERROR)
		{
			...
			{
                memset(reply, '\0', 100);
                nError = endiSendRecv(endi, 0,ENDI_DATATYPE_BYPASS , szCommand, valuelen+5, reply2, replylen, 
                sensorCommTimeout(id, DEFAULT_COMM_TIMEOUT > 25000 ? DEFAULT_COMM_TIMEOUT : 25000));

				if (nError != CODIERR_NOERROR)
					XDEBUG("ERROR: endiWriteCID(%d): %s\r\n", nError, codiErrorMessage(nError));
				else{
					XDEBUG("SUCCESS: endiWriteCID(%d): %s\r\n", nError, codiErrorMessage(nError));
                   desuf_len = m_pKamstrupParser->ByteDeStuffing(reply, reply2, *replylen);
                    *replylen=desuf_len;
                     break;
                }
