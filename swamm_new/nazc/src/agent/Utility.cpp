...
void GetMemoryInfo(UINT *nTotal, UINT *nUse, UINT *nFree, UINT *nCache, UINT *nBuffer)
{
	char	*p, *pszBuffer;
	int		len, fd, nShare;
	...
	*nBuffer	= 0;
	...
	if (fd >= 0)
	{
		len = read(fd, pszBuffer, 1024);
		pszBuffer[len] = 0;
		if (len > 0)
		{
			p = strstr(pszBuffer, "Mem: ");
			if (p != NULL)
			{
				p += 5;
				sscanf(p, "%d%d%d%d%d%d",
					nTotal, nUse, nFree, &nShare, nBuffer, nCache);	
			}
		}
		close(fd);
	} 
...
void GetFlashInfo(UINT *nTotal, UINT *nUse, UINT *nFree)
{
	char	*p, *pszBuffer;
	int		len, fd;
	char	szCommand[100];
	...
	*nFree	= 0;
	...
	sprintf(szFileName, "/tmp/flash%p", szCommand);
	sprintf(szCommand, "/bin/df -k > %s", szFileName);
	system(szCommand);
	...
	if (fd >= 0)
	{
		...
		if (len > 0)
		{
			p = strstr(pszBuffer, "/dev/mtdblock2");
			if (p != NULL)
			{
				p += 14;
				sscanf(p, "%d%d%d", nTotal, nUse, nFree);
			}
		}
		close(fd);
	}
...
BOOL DestroyDirectories(void)
{
	...
	system("rm -f /app/conf/ethernet.type");
	system("rm -f /app/conf/mobile.*");
	system("rm -f /app/conf/passwd");
	system("rm -f /app/conf/patch.info");
	system("rm -f /app/conf/server.addr");
	system("rm -f /app/conf/user.conf");
	system("rm -f /app/conf/var.*");
	system("rm -rf /app/evtcfg");
	system("rm -rf /app/tr");
...
BOOL UpgradeConcentratorSystem(IF4Wrapper *pWrapper, BYTE UpgradeType, const char *szAddress, const char * upgradeFile)
{
    ...
    switch(UpgradeType)
    {
        case CONCENTRATOR_UPGRADE_GZIP:
	        ...
	        if((nError = system(szCommand)) != 0) 
            {
                /*-- gzip으로 풀다가 에러가 난 경우 --*/
                XDEBUG("Upgrade Error: uncompress fail ECODE=%d\r\n", nError);
                chdir("/app"); system("/bin/rm -rf /app/upgrade");
                return FALSE;
            }
            break;
...
void ExecutePPPD()
{
	...
	GetFileValue("/app/conf/mobile.type", &type);
