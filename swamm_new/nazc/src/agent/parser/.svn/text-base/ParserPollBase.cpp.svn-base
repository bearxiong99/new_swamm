...
/** LP를 읽어야 할 주소를 계산한다.
 * SX Meter등에서 다른 계산식을 사용해야 하기 때문에 virtual 로 분리한다.
 *
 */
WORD CPollBaseParser::GetLpAddress(BYTE nLpPointer, int nDay, BYTE nPeriod)
{
    return METER_LP_ADDRESS(ENDI_ROMAP_METER_LPLOGDATA, (nLpPointer-nDay), nPeriod);
}
int CPollBaseParser::GetLpSize(BYTE nPeriod)
{
    return nPeriod * LP_BLOCK_SIZE + 8;
}
    ...
    for(nRetry=0; (nRetry<GetRetryCount()) && (nState!=STATE_DONE);)
    {
        switch(nState) {
          ...
          case STATE_READ_LPDATA :
			    ...
			    nDay  = (nOffset - nSeek);
                addr = GetLpAddress(lpLog.LPPOINTER, nDay, lpLog.LPPERIOD);
                nSize = GetLpSize(lpLog.LPPERIOD);
            ...
            case STATE_OK :
                ...
                if (pOndemand->nResult == ONDEMAND_ERROR_OK)
                {
                    ...
                    /** Post Action */
                    ActPostSuccAction(pChunk, pOndemand, (void *)&lpLog);
                ...
                {
                    /** Post Action */
                    ActPostFailAction(pChunk, pOndemand, (void *)&lpLog);
