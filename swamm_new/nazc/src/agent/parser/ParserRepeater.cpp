...
int CRepeaterParser::REPEATER_MAIN(CMDPARAM *pCommand, METER_STAT *pStat)
{
    ...
    switch(pCommand->nOption) {
        ...
        /*case REPEATER_OPTION_READ_INCREMENTAL_SOLAR_LOG : */
            ...
            break;
        default :
            nOption = DEFAULT_METERING_OPTION;
            break;
    }

    if(pCommand->nOption == -1) nOffset = 1;
    else nOffset = pCommand->nOffset > -1 ? pCommand->nOffset : 1;
	if (nOffset > 13) nOffset = 13;

	nCount = pCommand->nCount <= 14  ? pCommand->nCount : 1;
