...
WORD m_nKmpReadIdx = 0;
WORD m_nKmpLastIdx = 0;
...
BOOL GetShellCommandResult(CChunk *pChunk, char *pszCommand)
{
	...
	if (!pszCommand || !pChunk)
		return FALSE;

    sprintf(szFileName, "/tmp/result%p", pszCommand);
    sprintf(szCommand, "%s > %s", pszCommand, szFileName);
    system(szCommand);
                ...
                switch(tnum) {
					...
					case 23: {
                        ...
                        summation_ch2 = CondStreamToHostLong(nOrder, &ts->TOT_DATA_BLOCK.SUMMATIONS[6], 6);
                       	PRINTOUT("                + TOT_SUM_CH1 %lld (%.3Lf) TOT_SUM_CH2 %lld (%.3Lf)\xd\xa",
                       	    summation_ch1, (long double)summation_ch1 / 400000, 
                            summation_ch2, (long double)summation_ch2 / 400000);
                        }
                        else // Demand
                        {
                    ...
                    case 64: {
                        ...
                        for(i=0;i<tlen;i+=lpLen) {
                            ...
                            PRINTOUT("                + BLK_END_TIME %04d/%02d/%02d %02d:%02d\xd\xa", 
                            ...
                            tidx += sizeof(ANSI_END_READINGS) * nbrChnsSet1;      
                            ...
                            for(j=0;j<nbrBlkIntsSet1;j++)
                            {
                                ...
                                for(k=0;k<nbrChnsSet1;k++)
                                {
                                    ...
                                    if(v == 0xFFFF)
                                    {
                                        ...
                                        dv = (long double)CondToHostShort(nOrder, v) * (long double)vAhSf * pow(10, -9) *
	    ...
	    switch(pTag->tag) {
	      ...
	      case DLMS_DATATYPE_FLOAT32 :
			   PRINTOUT("%s %02d: FLOAT32(%d), length=4, value=%f\r\n", prefix, n+1, pTag->tag, 
						(double)BigToHostInt(pArg->var.u32));
			   break;
...
const char *dlms_result_total_tariff[] = {"Tot", "Tr1", "Tr2", "Tr3"};
...
#endif
{
    ...
    dlmsToken(pToken, nToken, &nToken, 100, data, nLen);
            ...
            switch(attr)
            {
                case 2: // value
                    if(!memcmp(obis, &_device_info, sizeof(OBISCODE))) {
                        ...
                        memset(szBuffer, 0, sizeof(szBuffer));
                        strncpy(szBuffer, (char *)pToken[0].var.p, MIN(pToken[0].len, sizeof(szBuffer)-1));
                        PRINTOUT(DLMS_GAP "ManufacturerMeterId %s\r\n", szBuffer);
                        return TRUE;
                    }
                    break;
            }
            ...
            switch(attr)
            {
                case 2: // value
                    if(!memcmp(obis, &_meter_constant_active, sizeof(OBISCODE))) {
                        if(nToken < 1) break;
                        PRINTOUT(DLMS_GAP "ActiveC %6.3f\r\n", BigOctetToHostFloat((char *)pToken[0].var.p));
                        return TRUE;
            ...
            switch(attr)
            {
                case 2: // buffer
                    if(!memcmp(obis, &_monthly_enery_profile, sizeof(OBISCODE)-1)) {        // VZ 부분은 검사 안함
                        ...
                        {
                            PRINTOUT(DLMS_GAP "%s Active %6u Reactive %6u AvgPFactor %6.3f\r\n", 
                                    dlms_result_total_tariff[n], 
                                    pToken[i].var.u32, pToken[i+1].var.u32, 
                                    BigOctetToHostFloat((char *)pToken[i+2].var.p));
                        }
                        return TRUE;
                        ...
                        for(i=n-1,n=1; i<nToken; i+=n, n=1)
                        {
                            ...
                            if(pToken[i+1].type == DLMS_DATATYPE_UINT16 &&
                               ...
                               pToken[i+6].type == DLMS_DATATYPE_BIT_STRING)
                            {
                                ...
                                PRINTOUT(DLMS_GAP "%04d/%02d/%02d %02d:%02d:%02d 0x%02X %4d %4d %4d %4d\r\n", 
                                        BigToHostShort(pDTime->year), pDTime->mon, pDTime->day,
                                        pDTime->hour, pDTime->min, pDTime->sec, pToken[i+6].var.p[0],
                                        pToken[i+1].var.u16, pToken[i+2].var.u16,
                                        pToken[i+3].var.u16, pToken[i+4].var.u16);
                            }else if(pToken[i+1].type == DLMS_DATATYPE_UINT16 && pToken[i+2].type == DLMS_DATATYPE_UINT16)
                            {
                                /** 일진 미터 */
                        ...
                        /*
                        for(n=0, len=0;n<nToken;n++)
                        {
                            if(pToken[n].type == DLMS_DATATYPE_ARRAY) 
                            {
                                len = pToken[n].var.array;
                                break;
                            }
                        }
                        */
...
#endif
{
    ...
    pToken = (DLMSVARIABLE *)MALLOC(sizeof(DLMSVARIABLE)*100);
    if(pToken==NULL)
    {
...
const char * getVendorString(BYTE nVendor)
{
    switch (nVendor)
    {
        case METER_VENDOR_KAMSTRUP:     return "KAMSTRUP";
