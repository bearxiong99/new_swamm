...
BOOL CMD5::GetString(const char *pszSource, char *pszDest)
{
	MD5_CTX context;
	char	temp[5];
	BYTE	digest[16];
	...
	*pszDest = '\0';
	...
	MD5Init(&context);
	MD5Update(&context, (const BYTE *)pszSource, len);
...
void CMD5::MD5Update(MD5_CTX *context, const BYTE *input, UINT inputLen)
{
	...
	if (inputLen >= partLen) 
	{
        ...
        inputSp = const_cast<BYTE *>(input);
		
		for (i = partLen; i + 63 < inputLen; i += 64)
			MD5Transform (context->state, &inputSp[i]);
...
void CMD5::MD5Final(BYTE *digest, MD5_CTX *context)
{
	BYTE bits[8];
	UINT index, padLen;
