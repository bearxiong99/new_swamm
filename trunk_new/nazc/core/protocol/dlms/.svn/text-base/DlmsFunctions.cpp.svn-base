...
int dlmsMakeGetFrame(DLMSOBIS *obis, DLMSTAGVALUE *pParam, BYTE lastRxControl, BYTE src, UINT dest, BYTE **pszDlmsFrame, int *nFrameLength)
{
	...
	payload.method		= DLMS_GET_REQUEST;
...
int dlmsMakeSetFrame(DLMSOBIS *obis, DLMSTAGVALUE *pParam, BYTE lastRxControl, BYTE src, UINT dest, BYTE **pszDlmsFrame, int *nFrameLength)
{
	DLMS_FRAME	frame;
	DLMS_SETREQ_PAYLOAD	payload;
	WORD        nSize=0;
	BYTE		nControl;
    int         nError;
    BYTE        nNull = 0;

	memset(&payload, 0, sizeof(DLMS_SETREQ_PAYLOAD));
    payload.destLSAP    = LSAP_DEST_ADDRESS;
    payload.srcLSAP     = LSAP_SRC_ADDRESS;
	payload.method		= DLMS_SET_REQUEST;
	payload.reqFlag		= DLMS_NORMAL;
	payload.idPriority	= DLMS_INVOKE_ID | DLMS_INVOKE_PRIORITY;
	payload.cls			= BigToHostShort(obis->cls);
	payload.attr		= obis->attr;
...
int dlmsMakeActionFrame(DLMSOBIS *obis, DLMSTAGVALUE *pParam, BYTE lastRxControl, BYTE src, UINT dest, BYTE **pszDlmsFrame, int *nFrameLength)
{
	DLMS_FRAME	frame;
	DLMS_ACTIONREQ_PAYLOAD	payload;
	WORD        nSize=0;
	BYTE		nControl;
    int         nError;
    BYTE        nNull = 0;

	memset(&payload, 0, sizeof(DLMS_ACTIONREQ_PAYLOAD));
    payload.destLSAP    = LSAP_DEST_ADDRESS;
    payload.srcLSAP     = LSAP_SRC_ADDRESS;
	payload.method		= DLMS_ACTION_REQUEST;
	payload.reqFlag		= DLMS_NORMAL;
	payload.idPriority	= DLMS_INVOKE_ID | DLMS_INVOKE_PRIORITY;
	payload.cls			= BigToHostShort(obis->cls);
	payload.actMethod	= obis->attr;   // Attribute 값을 method 로 사용한다
	memcpy(&payload.obis, &obis->code, sizeof(OBISCODE));	
	
	nControl = GetHdlcControl(lastRxControl, FALSE);
	if((nError=MakeDlmsGenericFrame((BYTE *)&frame, src, dest, nControl,
								(BYTE *)&payload, sizeof(DLMS_ACTIONREQ_PAYLOAD), 
                                pParam == NULL ? &nNull : pParam->pPoint,
                                pParam == NULL ? 1 : pParam->nLength, &nSize))!=DLMSERR_NOERROR) 
    {
        return nError;
