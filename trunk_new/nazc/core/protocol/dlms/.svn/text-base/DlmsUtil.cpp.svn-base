...
int DlmsSetTagValue(DLMSTAGVALUE * tagBuf, BYTE nType, void *pValue, BYTE nLen)
{
	...
	switch(nType) {
	      ...
	      case DLMS_DATATYPE_UINT32 :
			   ...
			   break;
	      case DLMS_DATATYPE_FLOAT32 :
               if(nLen != 4) return DLMSERR_INVALID_PARAM;
               fv = dlmsOrderByFloat(*(float *)pValue);
               memcpy(&tagBuf->pPoint[tagBuf->nLength], (float *)&fv, nLen);
               tagBuf->nLength += nLen;
			   break;
  	      case DLMS_DATATYPE_INT64 :
	      case DLMS_DATATYPE_UINT64 :
               ...
               memcpy(&tagBuf->pPoint[tagBuf->nLength], (BYTE *)&uuv, nLen);
               tagBuf->nLength += nLen;
			   break;
		  default :
...
#ifdef      __DEBUG_DLMS_LIB__
               XDEBUG(" DLMS Token [%02d] : ARRAY %d\r\n", n, pValue->var.array);
#elif defined __STDERR_DLMS_LIB__
               fprintf(stderr," DLMS Token [%02d] : ARRAY %d\r\n", n, pValue->var.array);
#endif
			   break;
...
#elif defined __STDERR_DLMS_LIB__
               fprintf(stderr," DLMS Token [%02d][%d] : STRUCTURE\r\n", n, pArg->var.u8);
#endif
			   break;
...
#elif defined __STDERR_DLMS_LIB__
               fprintf(stderr," DLMS Token [%02d] : BIT-STRING\r\n", n);
#endif
               break;
...
#elif defined __STDERR_DLMS_LIB__
               fprintf(stderr," DLMS Token [%02d] : STRING\r\n", n);
#endif
			   break;
...
#elif defined __STDERR_DLMS_LIB__
               fprintf(stderr," DLMS Token [%02d] : DATE TIME\r\n", n);
#endif
			   break;
...
#elif defined __STDERR_DLMS_LIB__
               fprintf(stderr," DLMS Token [%02d] : DATE\r\n", n);
#endif
			   break;
...
#elif defined __STDERR_DLMS_LIB__
               fprintf(stderr," DLMS Token [%02d] : TIME\r\n", n);
#endif
			   break;
...
#elif defined __STDERR_DLMS_LIB__
               fprintf(stderr," DLMS Token [%02d] : BCD\r\n", n);
#endif
			   break;
...
#elif defined __STDERR_DLMS_LIB__
               fprintf(stderr," DLMS Token [%02d] : INT8\r\n", n);
#endif
               break;
...
#elif defined __STDERR_DLMS_LIB__
               fprintf(stderr," DLMS Token [%02d] : UINT8\r\n", n);
#endif
			   break;
...
#elif defined __STDERR_DLMS_LIB__
               fprintf(stderr," DLMS Token [%02d] : UINT16\r\n", n);
#endif
			   break;
...
#elif defined __STDERR_DLMS_LIB__
               fprintf(stderr," DLMS Token [%02d] : UINT32\r\n", n);
#endif
			   break;
	      case DLMS_DATATYPE_FLOAT32 :
			   pValue->len			= 4;
			   pValue->var.f32		= dlmsOrderByFloat(pArg->var.f32);
#ifdef      __DEBUG_DLMS_LIB__
               XDEBUG(" DLMS Token [%02d] : FLOAT32\r\n", n);
#elif defined __STDERR_DLMS_LIB__
               fprintf(stderr," DLMS Token [%02d] : FLOAT32\r\n", n);
#endif
               break;
...
#elif defined __STDERR_DLMS_LIB__
               fprintf(stderr," DLMS Token [%02d] : UINT64\r\n", n);
#endif
			   break;
...
float dlmsOrderByFloat(float value)
{
	return BigToHostFloat(value);
}
...
const char *dlmsGetConfirmedServiceError(BYTE confirmedServiceError)
{
    switch(confirmedServiceError)
    {
        case CONF_INITIATE_ERR: return "Initiate";
        case CONF_READ_ERR: return "Read";
        case CONF_WRITE_ERR: return "Write";
    }

	return "UNKNOWN";
}

const char *dlmsGetServiceError(BYTE serviceError, BYTE detail)
{
    if(detail == 0)
    {
        return "other";
    ...
    switch(serviceError)
    {
        case 0: // application reference[0]
            switch(detail)
            {
                 case SERVICE_ERR_TIME_ELAPSED: return "time-slapsed";
                 case SERVICE_ERR_APPLICATION_UNREACHABLE: return "app-unreachable";
                 case SERVICE_ERR_APPLICATION_REFERENCE_INVALID: return "app-reference-invalid";
                 case SERVICE_ERR_APPLICATION_CONTEXT_UNSUPPORTED: return "app-context-unsupported";
                 case SERVICE_ERR_PROVIDER_COMMUNICATION: return "privder-comm-err";
                 case SERVICE_ERR_DECIPHERING: return "deciphering-err";
            }
            break;
        case 1: // hardware-resource[1]    
            switch(detail)
            {
                 case SERVICE_ERR_MEMORY_UNAVAILABLE: return "memory-unavailable";
                 case SERVICE_ERR_PROCESSOR_RESOURCE_UNACAILABLE: return "process-resource-unavailable";
                 case SERVICE_ERR_MASS_STORAGE_UNAVAILABLE: return "mass-storage-unavailable";
                 case SERVICE_ERR_OTHER_RESOURCE_UNAVAILABLE: return "other-resource-unavailable";
            }
            break;
        case 2: // vde-state-error[2]
            switch(detail)
            {
                 case SERVICE_ERR_NO_DLMS_CONTEXT: return "no-dlms-context";
                 case SERVICE_ERR_LOADING_DATA_SET: return "loading-data-set";
                 case SERVICE_ERR_STATUS_NOCHANGE: return "status-nochagne";
                 case SERVICE_ERR_STATUS_INOPERABLE: return "status-inoperable";
            }
            break;
        case 3: // service[3]
            switch(detail)
            {
                 case SERVICE_ERR_PDU_SIZE: return "pdu-size-too-long";
                 case SERVICE_ERR_SERVICE_UNSUPPORTED: return "service-unsupported";
            }
            break;
        case 4: // definition[4]
            switch(detail)
            {
                 case SERVICE_ERR_UNDEFINED: return "obj-undefined";
                 case SERVICE_ERR_CLASS_INCONSISTENT: return "obj-class-inconsistent";
                 case SERVICE_ERR_ATTRIBUTE_INCONSISTENT: return "obj-attr-inconsistent";
            }
            break;
        case 5: // access[5]
            switch(detail)
            {
                 case SERVICE_ERR_SCOPE_OF_ACCESS_VIOLATED: return "scope-of-access-violated";
                 case SERVICE_ERR_OBJECT_ACCESS_INVALID: return "obj-access-invalid";
                 case SERVICE_ERR_HARDWARE_FAULT: return "hw-fault";
                 case SERVICE_ERR_OBJECT_UNAVAILABLE: return "obj-unavailable";
            }
            break;
        case 6: // initiate[6]
            switch(detail)
            {
                 case SERVICE_ERR_DLMS_VERSION_TOO_LOW: return "dlms-version-too-low";
                 case SERVICE_ERR_INCOMPATIBLE_CONFORMANCE: return "incompatible-conformance";
                 case SERVICE_ERR_PDU_SIZE_TOO_SHORT: return "pdu-size-too-short";
                 case SERVICE_ERR_REFUSED_BY_THE_VDE_HANDLER: return "refused-by-the-VDE";
            }
            break;
        case 7: // load-data-set[7]
            switch(detail)
            {
                 case SERVICE_ERR_PRIMITIVE_OUT_OF_SEQUENCE: return "primitive-out-of-seq";
                 case SERVICE_ERR_NOT_LOADABLE: return "not loadable";
                 case SERVICE_ERR_DATASET_SIZE_TOO_LARGE: return "dataset-size-too-large";
                 case SERVICE_ERR_NOT_AWAITED_SEGMENT: return "not-awaited-segment";
                 case SERVICE_ERR_INTERPRETATION_FAILURE: return "interpretation-failure";
                 case SERVICE_ERR_STORAGE_FAILURE: return "storage-failure";
                 case SERVICE_ERR_DATA_SET_NOT_READY: return "data-set-not-ready";
            }
            break;
        case 9: // task[9]
            switch(detail)
            {
                 case SERVICE_ERR_NO_REMOTE_CONTROL: return "no-remote-control";
                 case SERVICE_ERR_TI_STOPPED: return "ti-stopped";
                 case SERVICE_ERR_TI_RUNNING: return "ti-running";
                 case SERVICE_ERR_TI_UNUSABLE: return "ti-unusable";
            }
            break;
    }
