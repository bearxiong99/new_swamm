...
int CVARService::Load()
{
    ...
    /** Issue #53 
     *
     * CLI와 Agent가 동시에 실행되고 둘다 최초 실행이라 var.conf 가 없을 경우 CLI가
     * 먼저 실행되어 잘못된 var.conf 가 생성되는 경우가 발생한다. 이 문제를 방지하기
     * 위해 m_bSave 변수가 TRUE 일 때만 save 하게 되어 있었지만 두 곳의 save point 중
     * 이 곳에는 m_bSave 변수를 검사하지 않고 있었다. 따라서 CLI가 최초에 먼서 Initialize 되면
     * 모든 값이 NULL 인 var.conf가 생기는 문제가 있었다. 이 문제를 방지하기 위해 m_bSave를
     * 검사하도록 수정한다. 
     *
     * 스웨덴과 같이 생산 공정을 거치는 경우는 생산 초기 File 로 var.conf 가 제공되기 때문에
     * 이 문제가 발생하지 않았다. 이 문제는 초기화된 집중기에서 수동으로 Application을 설정할
     * 경우에만 발생할 수 있었다.
     *
     */
	if (nNewLength < 1024 && m_bSave == TRUE) 
	{
...
void CVARService::LoadFromFile(FILE *fp, VAROBJECT *pList)
{
	...
	{
		fread(item.stream.str, item.len, 1, fp);
