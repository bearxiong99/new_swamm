...
BOOL CEndDeviceList::SetMeteringAttribute(const EUI64 *id, int nPortNum, BYTE lastMask, BYTE period, BYTE choice, signed char alarmFlag)
{
    ...
    if(pEndDevice && bDbUpdate) {
        /** Issue #84 : DB Meter 정보 갱신 */
        m_pMeterHelper->Update(id, nPortNum, NULL, pEndDevice->nParser, pEndDevice->nServiceType, 
                -1, -1.0, -1.0, period);
    }
        ...
        UINT nParserType, int nAttribute)
{
    ...
    if(pEndDevice) {
        /** Issue #84 : DB Meter 정보 갱신 */
        m_pMeterHelper->Update(id, nPortNum, NULL, nParserType, pEndDevice->nServiceType, -1, -1.0, -1.0, -1);
        if(pParser){
...
BOOL CEndDeviceList::SetEndDeviceFlag(const EUI64 *id, WORD alarmMask, signed char alarmFlag, BOOL testFlag)
{
	ENDIENTRY	*pEndDevice;
	...
	if (pEndDevice != NULL)
	{
        if(pEndDevice->nAlarmMask != alarmMask ) {
		    pEndDevice->nAlarmMask = alarmMask;
			pEndDevice->bModified = TRUE;
        }
        if(alarmFlag >= 0 && pEndDevice->nAlarmFlag != alarmFlag ) {
		    pEndDevice->nAlarmFlag = alarmFlag;
        ...
        const char *pszModel, const char *pszSerial, WORD hw, WORD fw)
{
    ...
    if(pEndDevice != NULL) {
        ...
        m_pMeterHelper->Update(id, 
                nPortNum,       // nPortNum
                stripedSerial,  // szSerial 
                nParser,        // nParser Type
                pEndDevice->nServiceType,
                nVendor, 
                (double)  ((hw >> 8) + (hw & 0xFF) / 10.0),
                (double)  ((fw >> 8) + (fw & 0xFF) / 10.0),
                -1
        );
    }
...
BOOL CEndDeviceList::DeleteEndDeviceByID(const EUI64 *id)
{
	...
	if (pEndDevice != NULL)
	{
        ...
        /** Issue #84: DB Table에서 대상을 삭제한다 */
        /** Modem Table을 지우면 관련된 Table을 같이 삭제한다 */
        m_pModemHelper->Delete(id);
	}
	m_Locker.Unlock();
	return pEndDevice ? TRUE : FALSE;
}
...
BOOL CEndDeviceList::DeleteEndDeviceAll()
{
    ...
    /** Issue #84: DB Table에서 대상을 삭제한다 */
    /** Modem Table을 지우면 관련된 Table을 같이 삭제한다 */
    m_pModemHelper->Delete((char *)NULL);
...
/** Issue #930 개별 ID에 대한 Property Value를 조회할 수 있게 한다.
 *
 * Issue #320에 의해 GetMeterSecurity 로 만들어진 함수를 좀더 일반적인 GetMeterProperty로
 * 변경한다.
 *
 * @param szGroup Property Group
 * @param szProperty 각 Propery Group에 맞는 Property Name
 * @param id
 * @param szBuffer Meter Security가 들어갈 Buffer. method 내부에서 검사해 주지 
 *          않기 때문에 충분한 공간을 확보한 후 호출해야 한다.
 * @param nLength key 길이
 *
 * 현재 특별하게 처리하고 있는 Property Group 및 Property Name
 *
 * <ul>
 *  <ul> PROPERTY_GRP_SECURITY
 *    <li> PROPERTY_NAME_SECURITY
 *  </ul>
 *  <ul> PROPERTY_GRP_METERING_PARAMETER
 *    <li> PROPERTY_NAME_OPTION
 *    <li> PROPERTY_NAME_OFFSET
 *    <li> PROPERTY_NAME_COUNT
 *  </ul>
 *  <ul> PROPERTY_GRP_DLMS_ADDRESS
 *    <li> PROPERTY_NAME_DLMS_SERVER_ADDR
 *    <li> PROPERTY_NAME_DLMS_BROADCAST
 *    <li> PROPERTY_NAME_DLMS_CLIENT_ADDR
 *  </ul>
 * </ul>
 *
 */
BOOL CEndDeviceList::GetMeterProperty(const char *szGroup, const char *szProperty, const EUI64 *id, BYTE* szBuffer, int* nLength)
{
	ENDIENTRY	*pEndDevice;
    ...
    m_pMeterPropertyHelper->Select(szGroup, szProperty, pEndDevice->szParser, pEndDevice->szNodeKind, &pEndDevice->id, &fetchData);

    if(fetchData.nCount > 0)
    {
        Row* pRow = m_pMeterPropertyHelper->GetHeadRow(&fetchData);
        METERPROPERTYDATAINS* ins = m_pMeterPropertyHelper->GetData(pRow);

        if(ins != NULL)
        {
            if(ins->pValue != NULL && ins->nValueLen > 0)
            {
                if(nLength != NULL)
                {
                    *nLength = ins->nValueLen;
                }
                memcpy(szBuffer, ins->pValue, ins->nValueLen);
                m_pMeterPropertyHelper->FreeResult(&fetchData);
                return TRUE;
            }
            m_pMeterPropertyHelper->FreeResult(&fetchData);
        }
...
{
    return GetMeterProperty(PROPERTY_GRP_SECURITY, PROPERTY_NAME_SECURITY, id, szBuffer, nLength);
}

/** 해당 Device에 Metering Option이 설정되어 있다면 정보를 얻어온다 (Issue #855).
 * Metering Option은 id, node kind, parser type 의 우선순위로 검색하게 되며
 * Metering Option이 select 되었을 때 TRUE, 실패했거나 없을 경우 FALSE를 리턴한다.
 *
 * @param id
 * @param nOption 
 * @param nOffset 
 ...
 *
 */
BOOL CEndDeviceList::GetMeteringOption(const EUI64 *id, int *nOption, int *nOffset, int *nCount)
{
	ENDIENTRY	*pEndDevice;
    BYTE szBuffer[64];
    int nBufferLen;
    ...
    if(id == NULL || (nOption == NULL && nOffset == NULL && nCount == NULL)) return FALSE;
    ...
    memset(szBuffer, 0, sizeof(szBuffer)); nBufferLen = 0;
    if(GetMeterProperty(PROPERTY_GRP_METERING_PARAMETER, PROPERTY_NAME_OPTION, id, szBuffer, &nBufferLen))
    {
        if(nOption != NULL) 
        {
            *nOption = strtol((char *)szBuffer, (char **)NULL, 10);
            bRet = TRUE;
        }
    }

    memset(szBuffer, 0, sizeof(szBuffer)); nBufferLen = 0;
    if(GetMeterProperty(PROPERTY_GRP_METERING_PARAMETER, PROPERTY_NAME_OFFSET, id, szBuffer, &nBufferLen))
    {
        if(nOffset != NULL) 
        {
            *nOffset = strtol((char *)szBuffer, (char **)NULL, 10);
            bRet = TRUE;
        }
    }

    memset(szBuffer, 0, sizeof(szBuffer)); nBufferLen = 0;
    if(GetMeterProperty(PROPERTY_GRP_METERING_PARAMETER, PROPERTY_NAME_COUNT, id, szBuffer, &nBufferLen))
    {
        if(nCount != NULL) 
        {
            *nCount = strtol((char *)szBuffer, (char **)NULL, 10);
            bRet = TRUE;
        }
    }
