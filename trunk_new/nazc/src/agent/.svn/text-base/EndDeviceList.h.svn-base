...
public:
    ...
    BOOL    SetEndDeviceFlag(const EUI64 *id, WORD alarmMask, signed char alarmFlag, BOOL testFlag);
    ...
    BOOL    GetMeterProperty(const char *szGroup, const char *szProperty, const EUI64 *id, BYTE* szBuffer, int* nLength);
    BOOL    GetMeterSecurity(const EUI64 *id, BYTE* szBuffer, int* nLength);
    BOOL    GetMeteringOption(const EUI64 *id, int *nOption, int *nOffset, int *nCount);
