...
CMeterParserFactory::CMeterParserFactory()
{
    ...
    /** SX에 SubCore Parser를 연결 */
    m_sx1Parser.SetSubCoreParser(&m_sxSubCoreParser);
}
...
CMeterParser *CMeterParserFactory::FindParser(const char *pszType)
{
	...
	if(IsMatchedParser(m_sx1Parser, pszType)) return &m_sx1Parser;
	if(IsMatchedParser(m_sxSubCoreParser, pszType)) return &m_sxSubCoreParser;
    if(IsMatchedParser(m_3rdPartyParser, pszType)) return &m_3rdPartyParser;
    if(IsMatchedParser(m_kukdongParser, pszType)) return &m_kukdongParser;
