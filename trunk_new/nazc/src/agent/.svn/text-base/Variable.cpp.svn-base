...
#define MOBILE_MODULE_PATH_FILE         "/app/conf/mobile.module.type"
...
#else
int     m_nMobileMode               = MOBILE_MODE_ALWAYS;
#endif
...
/** Issue #855 : cmdGetMeteringOption(104.18), cmdSetMeteringOption(104.19)를 추가하여 deprecated 되었다. */
...
// OID 2.12 sysMobileCfgEntry
MOBILECFGENTRY m_sMobileConfig = {
#ifdef __TI_AM335X__                // NZC4
/** TODO : 국낸 LTE 사용 시 MOBILE_MODULE_TYPE_AME5210 이 Default로 적용되어야 하는 경우가 있다.
  * 이 문제를 해결하기 위해 MOBILE_TYPE_CDMA 일 경우 AME5210으로 셋팅하는 루틴이 필요하다.
  */
            MOBILE_MODULE_TYPE_GE910,       // Module Type : TI의 경우 GSM 2G Telit
            0x00A0,                         // Module Flag : pppd active, serial, flow control
            "/dev/ttyO1",                   // Interface
            115200,                         // Interface Speed
            "",                             // SMS
            0                               // SMS Speed
#else
            MOBILE_MODULE_TYPE_MC55,        // Module Type : PAX의 경우 GSM 2G Simens
            0x0080,                         // Module Flag : pppd active, serial
            "/dev/ttyS00",                  // Interface
            115200,                         // Interface Speed
            "",                             // SMS
            0                               // SMS Speed
#endif
...
VAROBJECT    m_mcu_node[] =
{
    ...
    {  12,    "mobileCfgEntry", { {2,12,0}, VARSMI_OPAQUE, sizeof(MOBILECFGENTRY) },
                                VARATTR_RW | VARATTR_LIST, m_mobileCfgEntry_node, NULL, mobileCfgEntryQuery },
...
VAROBJECT    m_mobileCfgEntry_node[] =
{
    {   1,    "moduleType", { {2,12,1}, VARSMI_STRING, 32 },          VARATTR_RW, NULL, NULL, mobileCfgEntryQuery },
    {   2,    "moduleFlag", { {2,12,2}, VARSMI_UINT, 4 },             VARATTR_RW, NULL, NULL, mobileCfgEntryQuery },
    {   3,    "interface", { {2,12,3}, VARSMI_STRING, 64 },           VARATTR_RW, NULL, NULL, mobileCfgEntryQuery },
    {   4,    "interfaceSpeed", { {2,12,4}, VARSMI_UINT, 4 },         VARATTR_RW, NULL, NULL, mobileCfgEntryQuery },
    {   5,    "sms", { {2,12,5}, VARSMI_STRING, 64 },                 VARATTR_RW, NULL, NULL, mobileCfgEntryQuery },
    {   6,    "smsSpeed", { {2,12,6}, VARSMI_UINT, 4 },               VARATTR_RW, NULL, NULL, mobileCfgEntryQuery },
    {   0,    NULL }
};
...
VAROBJECT    m_eventMcu_node[] =
{
    ...
    {  30,    "eventMcuBatteryFail", { {200,30,0}, VARSMI_OID, 0 },       VARATTR_READ, NULL, NULL, NULL },
...
VAROBJECT    m_eventSensor_node[] =
{
    ...
    { 109,    "eventSensorEnergyLevelChange", { {203,109,0}, VARSMI_OID, 0 }, VARATTR_READ, NULL, NULL, NULL },
    {   0,    NULL }
};
...
int sysEntryQuery(VAROBJECT *pObject, int nMethod, MIBValue *pValue, MIBValue **pList, int *nCount, MIBValue *pParam)
{
    switch(pObject->var.oid.id3) {
      ...
      case 8 :
           ...
           if (nMethod == VARMETHOD_DEFAULT)
               pObject->var.stream.u8 = m_nHwVersion;
           else if (nMethod == VARMETHOD_GET)
               VARAPI_CopyValue(pObject, pValue);
           else if (nMethod == VARMETHOD_MOVE)
               pValue->stream.u8 = m_nHwVersion;
           return 1;
      case 9 :
...
#if defined(__TI_AM335X__)      // NZC4
               sprintf(buffer,"cp -f /app/sw/zoneinfo/GMT%d /app/init/localtime 2> /dev/null", tzvalue);
#else                           // NZC1, NZC2
...
int varValueEntryQuery(VAROBJECT *pObject, int nMethod, MIBValue *pValue, MIBValue **pList, int *nCount, MIBValue *pParam)
{
    switch(pObject->var.oid.id3) {
      ...
      case 81 :
           ...
           return 1;
      case 82 : // Issue #855 : cmdGetMeteringOption(104.18), cmdSetMeteringOption(104.19)를 추가하여 deprecated 되었다
...
int mobileCfgEntryQuery(VAROBJECT *pObject, int nMethod, MIBValue *pValue, MIBValue **pList, int *nCount, MIBValue *pParam)
{
    ...
    switch(pObject->var.oid.id3) {
      case 0 :
           // sysMobileCfgEntry(2.12.0) OPAQUE Read/Write
           if (nMethod == VARMETHOD_GET)
               VARAPI_CopyValue(pObject, pValue);
           else if (nMethod == VARMETHOD_MOVE)
           {
                 MOBILECFGENTRY    *pEntry;
                 VARAPI_CopyValue(pObject, pValue);
                 pEntry = (MOBILECFGENTRY *)pValue->stream.p;
                 if (pEntry != NULL)
                 {
                      memset(pEntry, 0, sizeof(MOBILECFGENTRY));
                      VARAPI_GetIndexValue(&pObject->pChild[0], (void *)&pEntry->moduleType, pParam);
                      VARAPI_GetIndexValue(&pObject->pChild[1], (void *)&pEntry->moduleFlag, pParam);
                      VARAPI_GetIndexValue(&pObject->pChild[2], (void *)&pEntry->interface, pParam);
                      VARAPI_GetIndexValue(&pObject->pChild[3], (void *)&pEntry->interfaceSpeed, pParam);
                      VARAPI_GetIndexValue(&pObject->pChild[4], (void *)&pEntry->sms, pParam);
                      VARAPI_GetIndexValue(&pObject->pChild[5], (void *)&pEntry->smsSpeed, pParam);
                 }
           }
           return 1;
      case 1 :
           // moduleType(2.12.1) STRING(32) Read/Write
           if (nMethod == VARMETHOD_DEFAULT)
           {
               sprintf(pObject->var.stream.p, "%s", m_sMobileConfig.moduleType);
               sprintf(moduleType,"%s", m_sMobileConfig.moduleType);
               SetFileString(MOBILE_MODULE_PATH_FILE, moduleType);
           }
           else if (nMethod == VARMETHOD_LOAD)
           {
               sprintf(m_sMobileConfig.moduleType, "%s", pObject->var.stream.p);
               sprintf(moduleType,"%s", m_sMobileConfig.moduleType);
               SetFileString(MOBILE_MODULE_PATH_FILE, moduleType);
           }
           else if (nMethod == VARMETHOD_GET)
           ...
           {
               sprintf(m_sMobileConfig.moduleType, "%s", pValue->stream.p);
               VARAPI_ReplaceValue(pObject, pValue);
               VARAPI_SetModify();
               sprintf(moduleType,"%s", m_sMobileConfig.moduleType);
               SetFileString(MOBILE_MODULE_PATH_FILE, moduleType);
           ...
           return 1;
      case 2 :
           // moduleFlag(2.12.2) UINT(4) Read/Write
           if (nMethod == VARMETHOD_DEFAULT)
           {
               pObject->var.stream.u32 = m_sMobileConfig.moduleFlag;
           }
           else if (nMethod == VARMETHOD_LOAD)
           {
               m_sMobileConfig.moduleFlag = pObject->var.stream.u32;
           }
           else if (nMethod == VARMETHOD_GET)
           {
               VARAPI_CopyValue(pObject, pValue);
           ...
           {
               m_sMobileConfig.moduleFlag = pValue->stream.u32;
               VARAPI_ReplaceValue(pObject, pValue);
               VARAPI_SetModify();
           }
           return 1;
      case 3 :
           // interface(2.12.3) STRING(64) Read/Write
           if (nMethod == VARMETHOD_DEFAULT)
           {
               sprintf(pObject->var.stream.p, "%s", m_sMobileConfig.interface);
           }
           else if (nMethod == VARMETHOD_LOAD)
           {
               sprintf(m_sMobileConfig.interface, "%s", pObject->var.stream.p);
           }
           else if (nMethod == VARMETHOD_GET)
           {
               VARAPI_CopyValue(pObject, pValue);
           ...
           {
               sprintf(m_sMobileConfig.interface, "%s", pValue->stream.p);
               VARAPI_ReplaceValue(pObject, pValue);
               VARAPI_SetModify();
           }
           return 1;
      case 4 :
           // interfaceSpeed(2.12.4) UINT(4) Read/Write
           if (nMethod == VARMETHOD_DEFAULT)
           {
               pObject->var.stream.u32 = m_sMobileConfig.interfaceSpeed;
           }
           else if (nMethod == VARMETHOD_LOAD)
           {
               m_sMobileConfig.interfaceSpeed= pObject->var.stream.u32;
           }
           else if (nMethod == VARMETHOD_GET)
           {
               VARAPI_CopyValue(pObject, pValue);
           ...
           {
               m_sMobileConfig.interfaceSpeed= pValue->stream.u32;
               VARAPI_ReplaceValue(pObject, pValue);
               VARAPI_SetModify();
           }
           return 1;
      case 5 :
           // sms(2.12.5) STRING(64) Read/Write
           if (nMethod == VARMETHOD_DEFAULT)
           {
               sprintf(pObject->var.stream.p, "%s", m_sMobileConfig.sms);
           }
           else if (nMethod == VARMETHOD_LOAD)
           {
               sprintf(m_sMobileConfig.sms, "%s", pObject->var.stream.p);
           }
           else if (nMethod == VARMETHOD_GET)
           {
               VARAPI_CopyValue(pObject, pValue);
           ...
           {
               sprintf(m_sMobileConfig.sms, "%s", pValue->stream.p);
               VARAPI_ReplaceValue(pObject, pValue);
               VARAPI_SetModify();
           }
           return 1;
      case 6 :
           // smsSpeed(2.12.6) UINT(4) Read/Write
           if (nMethod == VARMETHOD_DEFAULT)
           {
               pObject->var.stream.u32 = m_sMobileConfig.smsSpeed;
           }
           else if (nMethod == VARMETHOD_LOAD)
           {
               m_sMobileConfig.smsSpeed= pObject->var.stream.u32;
           }
           else if (nMethod == VARMETHOD_GET)
           {
               VARAPI_CopyValue(pObject, pValue);
           ...
           {
               m_sMobileConfig.smsSpeed= pValue->stream.u32;
               VARAPI_ReplaceValue(pObject, pValue);
               VARAPI_SetModify();
           }
           return 1;
    ...
    switch(pObject->var.oid.id3) {
...
#if defined(__TI_AM335X__)
                    GetFlashInfo("ubi0:filestore", &pEntry->flashTotalSize, &pEntry->flashUseSize, &pEntry->flashFreeSize);
#else
                    GetFlashInfo("/dev/mtdblock2", &pEntry->flashTotalSize, &pEntry->flashUseSize, &pEntry->flashFreeSize);
#endif
                 }
           }
           return 1;
      case 1 :
           ...
           else if (nMethod == VARMETHOD_MOVE)
           {
                 FLASHENTRY    flash;
#if defined(__TI_AM335X__)
                    GetFlashInfo("ubi0:filestore", &pValue->stream.u32, &flash.flashUseSize, &flash.flashFreeSize);
#else
                    GetFlashInfo("/dev/mtdblock2", &pValue->stream.u32, &flash.flashUseSize, &flash.flashFreeSize);
#endif
           }
           return 1;
      case 2 :
           ...
           else if (nMethod == VARMETHOD_MOVE)
           {
                 FLASHENTRY    flash;
#if defined(__TI_AM335X__)
                    GetFlashInfo("ubi0:filestore", &flash.flashTotalSize, &flash.flashUseSize, &pValue->stream.u32);
#else
                    GetFlashInfo("/dev/mtdblock2", &flash.flashTotalSize, &flash.flashUseSize, &pValue->stream.u32);
#endif
           }
           return 1;
      case 3 :
           ...
           else if (nMethod == VARMETHOD_MOVE)
           {
                 FLASHENTRY    flash;
#if defined(__TI_AM335X__)
                    GetFlashInfo("ubi0:filestore", &flash.flashTotalSize, &pValue->stream.u32, &flash.flashFreeSize);
#else
                    GetFlashInfo("/dev/mtdblock2", &flash.flashTotalSize, &pValue->stream.u32, &flash.flashFreeSize);
#endif
           }
           return 1;
    }
...
#else
                   pEntry->gpioDCD          = 0;     // Not support .diggo
                   pEntry->gpioRI           = 0 ; // GPIOAPI_ReadGpio(GP_GSM_RI_INPUT); // Not support .diggo
#endif
...
#else
                   pEntry->gpioHeaterState  = 0 ; //GPIOAPI_ReadGpio(GP_TEMP_OS_INPUT);
                   pEntry->gpioCoverState   = 0 ; //GPIOAPI_ReadGpio(GP_DOOR_OPEN_INPUT);
#endif
...
int gpioCtrlEntryQuery(VAROBJECT *pObject, int nMethod, MIBValue *pValue, MIBValue **pList, int *nCount, MIBValue *pParam)
{
    ...
    switch(pObject->var.oid.id3) {
      ...
      case 12 :
           ...
           return 1;
      case 13 :
           // gpioLedDebug2(2.109.13) BYTE(1) Write Only
           if (nMethod == VARMETHOD_GET)
               VARAPI_CopyValue(pObject, pValue);
           else if (nMethod == VARMETHOD_SET)
      ...
      case 15 :
           ...
           if (nMethod == VARMETHOD_GET)
               ...
               GPIOAPI_WriteGpio(GP_DEBUG_LED4_OUT, pValue->stream.u8);
#endif
           return 1;
      case 16 :
...
int mobileEntryQuery(VAROBJECT *pObject, int nMethod, MIBValue *pValue, MIBValue **pList, int *nCount, MIBValue *pParam)
{
    switch(pObject->var.oid.id3) {
      ...
      case 3 :
           ...
           if (nMethod == VARMETHOD_LOAD)
           {
               char        pszBuffer[256], *p;
               FILE        *fp;
               int        n=0;

               fp = fopen("/app/conf/mobileinfo", "rb");
               if (fp != NULL)
               {
                   memset(pszBuffer, 0, sizeof(pszBuffer));
                   fread(pszBuffer, 1, sizeof(pszBuffer), fp);
                   p = strstr(pszBuffer, "CSQ ");
                   if (p != NULL)
                   {
                       n = strtol(p+4, (char**)NULL, 10);
                       m_nMobileCSQ = n;
           ...
           {
               pObject->var.stream.s32 = (m_nMobileCSQ>=99) ? 0 : -(113-(m_nMobileCSQ*2));
               VARAPI_CopyValue(pObject, pValue);
           }
           else if (nMethod == VARMETHOD_MOVE)
           {
