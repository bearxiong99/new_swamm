		...
		switch(nState) {
		  ...
		  case STATE_ONDEMAND :
			   ...
			   if (pItem->bDone)
			   {
                   ...
                   /** Issue #84 : DB Table에 Modem 정보를 추가한다 */
                   m_pModemHelper->Update(&pItem->id, szNodeKind, 
                           (double)  ((pItem->node.HARDWARE_VERSION >> 4) + (pItem->node.HARDWARE_VERSION & 0xf) / 10.0),
                           (double)  ((pItem->node.SOFTWARE_VERSION >> 4) + (pItem->node.SOFTWARE_VERSION & 0xf) / 10.0),
                           build);
