...
#define RTC_TEMP_GET	_IOWR('p', 0x15 , int)  /* Set PLL correction */
//////////////////////////////////////////////////////////////////////
...
BOOL CRealTimeClock::SetTime(TIMESTAMP *pTime)
{
    ...
    else
    {
        ...
        if ( !nRet )
        {
            ...
            {
                // Ramdisk v1.3
                SystemExec("hwclock -su");
            }
            else
            {
                // Ramdisk v2.2
                SystemExec("hwclock -su; hwclock -wl");
            }
    	    mcuClockChangeEvent();
        }
    }
...
int CRealTimeClock::GetTemperature()
{
	 int nValue = 0;
     int rtcFd = open("/dev/rtc" , O_RDWR | O_NOCTTY);
     if(rtcFd > 0)
     {
	    ioctl( rtcFd , RTC_TEMP_GET , &nValue );
        close(rtcFd);
     }
...
BOOL CRealTimeClock::UpdateTime(char *pszTime)
{
	...
	m_Locker.Lock();
    rtcFd = open("/dev/rtc" , O_RDWR | O_NOCTTY);
    if(rtcFd < 0)
    {
        if(m_szRtcNameSpace[0] != 0)
        {
    ...
    else
    {
        int nRet = 0;
        struct rtc_time rtc_tm;
        rtc_tm.tm_mday = tmNow.day;
  	    rtc_tm.tm_mon = tmNow.mon - 1;
  	    rtc_tm.tm_year = tmNow.year - 1900;
  	    rtc_tm.tm_hour = tmNow.hour;
  	    rtc_tm.tm_min = tmNow.min;
  	    rtc_tm.tm_sec = tmNow.sec;
        nRet = ioctl( rtcFd , RTC_SET_TIME , &rtc_tm );
        close(rtcFd);
        if ( !nRet )
        {
...
{
    /** OS Time을 이용해서 HW Clock을 동기화 하는 기능을 Disable 시킨다 */
	return TRUE;
}
...
BOOL CRealTimeClock::DetectRealTimeClock()
{
    ...
    {
        close(rtcFd);
        XDEBUG("RealTimeClock: /dev/rtc is %s.\xd\xa", m_szRtcNameSpace, "Installed" );
    } 
    else if(m_szRtcNameSpace[0] != 0)
    {
        XDEBUG("RealTimeClock: %s is %s.\xd\xa", m_szRtcNameSpace, m_bInstalled ? "Installed" : "Not installed");
    }
    else
    {
        XDEBUG("RealTimeClock: %s.\xd\xa", "Not installed");
    }
	return TRUE;
}
...
BOOL CRealTimeClock::GetTimeFromMobile()
{
	int		nResult;
	...
	XDEBUG("RealTimeClock: Starting mobile timesync.\r\n");

	// 모바일 정보를 구하면서 시간 동기화를 수행한다.
	MOBILE_Lock();
	nResult = MOBILE_GetMobileProperty();
	MOBILE_Unlock();
...
BOOL CRealTimeClock::TimesyncFromNetworkOrMobile()
{
    ...
    // Issue #1264 : 더이상 Mobile module에서 시간을 구하지 않는다.
	// CDMA는 서킷인 경우만 수행한다.
	// CDMA Packet/Always On은 접속시마다 시간 동기화 되어진다.
	if ((m_pService->GetMobileType() == MOBILE_TYPE_CDMA) &&
		(m_pService->GetMobileMode() == MOBILE_MODE_CSD))
		return GetTimeFromMobile();
#endif
