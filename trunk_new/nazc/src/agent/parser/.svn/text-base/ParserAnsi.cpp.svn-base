...
ANSITBLLIST m_AnsiMeterTblOption_0[] =
{
   ...
   //       Post Handler                    Description
    ...
    { ANSI_DEFAULT,             ANSI_STANDARD_TBL,  11, SAVE,   FULL_READ,        0,    
            GE_METER_TYPE_MASK_ALL ^ GE_METER_TYPE_MASK_AZOS,         GE_METER_MODE_ALL,
            NULL,                                   0,                  
    ...
    ////////////// TIME SYNC BLOCK BEGIN ////////////////////////
    { ANSI_DEFAULT,             ANSI_STANDARD_TBL,  52, SAVE,   FULL_READ,        7,    // Demand LP
            GE_METER_TYPE_MASK_ALL,         GE_METER_MODE_DEMAND_LP,
            NULL,                                   0,                  
            (void*)CANSIParser::fnPostS052BeforeTimesync,"Clock Table (Prev)"},
    { ANSI_DEFAULT,             ANSI_STANDARD_TBL,  55, SAVE,   FULL_READ,        9,    // TOU
            GE_METER_TYPE_MASK_ALL ^ GE_METER_TYPE_MASK_AZOS,         GE_METER_MODE_TOU,
            NULL,                                   0,                  
            (void*)CANSIParser::fnPostS052BeforeTimesync,"Clock State Table (Prev)"},
    ...
    { ANSI_DEFAULT,             ANSI_STANDARD_TBL,  52, SAVE,   FULL_READ,        7,    // Demand LP
            ...
            (void*)CANSIParser::fnPostS052AfterTimesync,"Clock Table (Curr)"},
    { ANSI_DEFAULT,             ANSI_STANDARD_TBL,  55, SAVE,   FULL_READ,        9,    
            GE_METER_TYPE_MASK_ALL ^ GE_METER_TYPE_MASK_AZOS,         GE_METER_MODE_TOU,
            (void*)CANSIParser::fnPreReadS055,                                    0,                  
            (void*)CANSIParser::fnPostS052AfterTimesync,"Clock State Table (Curr)"},
    ////////////// TIME SYNC BLOCK END ////////////////////////
    ...
    { ANSI_DEFAULT,             ANSI_STANDARD_TBL, 112, SAVE,   POFFSET_READ,     sizeof(ANSI_LOAD_CTRL_STATUS), // I210
            GE_METER_TYPE_MASK_I210 | GE_METER_TYPE_MASK_AZOS,        GE_METER_MODE_ALL,
            NULL,                                   0,                  
            (void*)CANSIParser::fnPostS112, "Relay status table(GE)"},
    { ANSI_DEFAULT,             ANSI_STANDARD_TBL, 130, SAVE,   FULL_READ,        2,    // SM110
            GE_METER_TYPE_MASK_SM110,       GE_METER_MODE_ALL,
            NULL,                                   0,                  
...
ANSITBLLIST m_AnsiMeterTblOption_8[] = // timesync meter
{
    ...
    { ANSI_DEFAULT,             ANSI_STANDARD_TBL,  55, SAVE,   FULL_READ,        9,    // TOU
            ...
            NULL,                                   0,                  
            (void*)CANSIParser::fnPostS052BeforeTimesync,"Clock State Table (Prev)"},
    { ANSI_DEFAULT,             ANSI_STANDARD_TBL,   7, SKIP,   PROC_WRITE,       0,    // Meter Timesync
            GE_METER_TYPE_MASK_ALL,         GE_METER_MODE_ALL,
...
{
	switch(pCommand->nMeteringType) {
        case METERING_TYPE_BYPASS:
		    if ((pCommand->pszParam == NULL) || (pCommand->nLength == 0))
...
int CANSIParser::GE_MAIN(CMDPARAM *pCommand, METER_STAT *pStat)
{
    ...
    /*-- Old Issue #1529 : GE Meter의 CLOCK_ERROR가 발생할 경우 무조건 Clear 시킨다 --*/
    /*-- Issue #855 : 이전 상태에 의해서 Clock Error Clear를 수행하는 조건 제외
      -- 이 Code가 살아 있으면 Clock Error를 Clear 시킨 후 다시 Clear 시키게 된다 (2중 Clear)
        ...
        UINT *nIndex, WORD *nRequestBytes)
{
    ...
    /** Issue #855 ~1960 : 일본지사에서 한 Block의 Instance 수를 극단적으로 1로 설정하였다. 이 경우 항상 Count가
      * 문제가 된다. 또한 이 때 강제적으로 nCount를 조정하기 때문에 이 부분을 Instance 수와 비교하여 계산되도록
      * 수정해야 한다
      */
    if(pMeterInfo->nLastBlockElement != 0 && nBlockIndex == 0 && pOndemand->nCount > pMeterInfo->nValidInstans)
    {
        nBlockIndex = (int)((pOndemand->nCount-pMeterInfo->nValidInstans) / pMeterInfo->nBlockIntsCount) + 1;
    }
...
/** Before Timesync
 *
 * Timesync 이전의 Standard 52 의 값
 */
void* CANSIParser::fnPostS052BeforeTimesync(ANSISESSION *pSession, ONDEMANDPARAM *pOndemand, BYTE nTableType, WORD nTableNumber, char *szTableData, WORD nTableLength)
{
    ANSI_ST_TABLE52 * pTable;
    ANSI_METER_INFO * pMeterInfo;
    ...
    pTable = (ANSI_ST_TABLE52 *)szTableData;

    memcpy(&pMeterInfo->currentTime, pTable, sizeof(ANSI_ST_TABLE52)); // currentTime이 ST55 Type이지만 ST52를 포함하기 때문에 문제 없다
...
/** After Timesync
 *
 * Timesync 이후의 Standard 52 의 값
 */
void* CANSIParser::fnPostS052AfterTimesync(ANSISESSION *pSession, ONDEMANDPARAM *pOndemand, BYTE nTableType, WORD nTableNumber, char *szTableData, WORD nTableLength)
{
...
void* CANSIParser::fnPostS061(ANSISESSION *pSession, ONDEMANDPARAM *pOndemand, BYTE nTableType, WORD nTableNumber, char *szTableData, WORD nTableLength)
{
    ...
    /** Issue #84 : Period 값 설정 
      */
    m_pMeterHelper->Update(&pOndemand->id, UNDEFINED_PORT_NUMBER, NULL, PARSER_TYPE_ANSI, SERVICE_TYPE_ELEC, 
                -1, -1.0, -1.0, (int)(60/pTable->MAX_INT_TIME_SET1));
...
void* CANSIParser::fnPostS062(ANSISESSION *pSession, ONDEMANDPARAM *pOndemand, BYTE nTableType, WORD nTableNumber, char *szTableData, WORD nTableLength)
{
    ...
    for(i=0;i<j;i++) {
        ...
        m_pChannelCfgHelper->Update(&pOndemand->id, UNDEFINED_PORT_NUMBER,
                i, // Channel Index
                ansiUnit2AimirUnit(pMeterInfo->uomEntryRcd[pLpSelSet1->LP_SOURCE_SELECT].ID_CODE), // nUnit
                0x41,  // SigExp : pow(10,-1)
                VALUE_TYPE_DIFF,
                OBJECT_TYPE_ELECTRICITY,  // Object Type
                ansiChannel2AimirChannel(pMeterInfo->uomEntryRcd[pLpSelSet1->LP_SOURCE_SELECT].ID_CODE,
                        pMeterInfo->uomEntryRcd[pLpSelSet1->LP_SOURCE_SELECT].TIME_BASE,
                        pMeterInfo->uomEntryRcd[pLpSelSet1->LP_SOURCE_SELECT].NET_FLOW_ACCOUNTABILITY,
                        pMeterInfo->uomEntryRcd[pLpSelSet1->LP_SOURCE_SELECT].Q1_ACCOUNTABILITY,
                        pMeterInfo->uomEntryRcd[pLpSelSet1->LP_SOURCE_SELECT].Q2_ACCOUNTABILITY,
                        pMeterInfo->uomEntryRcd[pLpSelSet1->LP_SOURCE_SELECT].Q3_ACCOUNTABILITY,
                        pMeterInfo->uomEntryRcd[pLpSelSet1->LP_SOURCE_SELECT].Q4_ACCOUNTABILITY
                    )
                );
 ...
 *
 */
BOOL CANSIParser::UpdateLastPulse(ORDER order, int nLastPulseSize, int nChannelCount, void *pEndiLastPulse, void *pLastPulse)
{
    /** TODO 실제 Meter에서 가지고 온 상수값들을 이용해서 계산해야 한다. 하지만 구현을 위한 시간이 부족해서
     *       알고 있는 상수값을 사용한다. 추후 관련 부분을 다시 구현해야 한다.
     */
    METERINGINS *pMeteringIns = (METERINGINS *)pLastPulse;
    unsigned long long magic = 400; // 2500(summations scale factor) * pow(10,-6) => ref(I210 4.1.1)
    ...
    magic /= 10;    // 기본 단위를 1/10 Wh 로 변경

    pMeteringIns->pLastValue = (unsigned long long *)MALLOC(size*nChannelCount);
    ptr = (char*)pMeteringIns->pLastValue;
    memset(ptr, 0, size * nChannelCount);

    for(i=0;i<nChannelCount;i++)
    {
        bigValue = HostToBigLong((unsigned long long)(CondStreamToHostLong(order, 
                        (BYTE *)pEndiLastPulse + (i*nLastPulseSize), nLastPulseSize) / magic)) ;
        memcpy(ptr + (i * size), &bigValue, size);
    }
 ...
 *
 */
BOOL CANSIParser::UpdateBasePulse(ORDER order, int nBasePulseSize, int nChannelCount, void *pEndiBasePulse, void *pBasePulse)
{
    /** TODO 실제 Meter에서 가지고 온 상수값들을 이용해서 계산해야 한다. 하지만 구현을 위한 시간이 부족해서
     *       알고 있는 상수값을 사용한다. 추후 관련 부분을 다시 구현해야 한다.
     */
    ...
    {
        bigValue = HostToBigLong((unsigned long long)(CondStreamToHostLong(order, 
                        (BYTE *)pEndiBasePulse + (i*nBasePulseSize), nBasePulseSize) / magic)) ;
        memcpy(ptr + (i * size), &bigValue, size);
    }
 ...
 *
 */
BOOL CANSIParser::UpdateLoadProfile(ORDER order, int nLpSize, int nChannelCount, int nInterval, int nLpCount, void *pEndiLp, void *pLoadProfile)
{
    /** TODO 실제 Meter에서 가지고 온 상수값들을 이용해서 계산해야 한다. 하지만 구현을 위한 시간이 부족해서
     *       알고 있는 상수값을 사용한다. 추후 관련 부분을 다시 구현해야 한다.
     */
    ...
    unsigned long long magic = 2; // 2500(summations scale factor) * pow(10,-6) * Divisor(200) => ref(I210 4.1.3)
    unsigned int bigValue;
    int size = sizeof(unsigned int);
    int i, j;
    int nExtendIntSize = (int)(nChannelCount / 2) + 1;
    int nSeek = 0;
    ...
    // 기본 단위를 1/10 Wh로 변경하기 위해 10으로 나눌경우 1이하가 되기 때문에 magic을 곱하는 것으로 변경
    magic = 5;      

    /** Modem에서 LP를 실제 값만 올리는것이 아니라 Extende Status 값도 올리고 있다. 
      * 따라서 값을 계산할 때 관련 부분을 제외하고 올린다.
      */
    memset(&timeStamp, 0, sizeof(TIMESTAMP));
    Timestamp5ToTimestamp(&pLp->lastTime, &timeStamp);
    timev = MakeTime(&timeStamp);
    pMeteringIns->nLpCnt = nLpCount;
    for(i=0;i<nLpCount;i++) {
        nSeek += nExtendIntSize;
        ...
        for(j=0;j<nChannelCount;j++,nSeek+=(nLpSize-1))
        {
            bigValue = HostToBigInt((unsigned int)(CondStreamToHostLong(order, 
                        pLp->lp + nSeek, nLpSize-1) * magic)) ;
            memcpy(ptr + (j * size), &bigValue, size);
        }
    }
 ...
 *
 */
BOOL CANSIParser::UpdateLog(ORDER order, int nEntryCount, TIMESTAMP *pCurrentTime, void *pEndiLog, void *pLogData)
{
    LOGENTRYINS *pLogEntryIns = (LOGENTRYINS *)pLogData;
    PUSH_EVENT *pEvent;
    ANSI_EVENT_ENTRIES *pAnsiEvt;
    TIMESTAMP6 timeStamp6;
    char * szData = (char *)pEndiLog;
    int i, j, idx, nSeek = 0;

    // 기본 값 설정은 상위 Class에서 수행
    CMeterParser::UpdateLog(order, nEntryCount, pCurrentTime, pEndiLog, pLogData);
    ...
    for(i=0,idx=0; i<nEntryCount; i++)
    {
        ...
        {
            pAnsiEvt = (ANSI_EVENT_ENTRIES *)(pEvent->data + (pEvent->eventLength * j));
            memcpy(&timeStamp6, &pAnsiEvt->EVENT_TIME, sizeof(ANSI_EVENT_TIME));    
            Timestamp6ToTimestamp(&timeStamp6, &pLogEntryIns[idx].logTime);
...
BOOL CANSIParser::IsPushMetering()
{
    return TRUE;
}
