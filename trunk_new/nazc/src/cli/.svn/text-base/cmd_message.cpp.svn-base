...
void MSGAPI_SysStateMask(UINT mask, char * buffer)
{
    sprintf(buffer, "%s%s%s%s%s%s%s",
            ...
            (mask & GPIONOTIFY_LOWBAT) ? "L" : "",
            (mask & GPIONOTIFY_BATTERY_FAIL) ? "B" : "",
            (mask & GPIONOTIFY_COORDINATOR) ? "C" : "",
            (mask & GPIONOTIFY_MEMORY) ? "M" : "",
            (mask & GPIONOTIFY_DOOR) ? "O" : "",
            (mask & GPIONOTIFY_FLASH) ? "D" : "");
}
...
void MSGAPI_GetKdAlarmStatus(BYTE nStatus, char * buffer)
{
    sprintf(buffer, "(");
    if(nStatus & 0x01) strcat(buffer, " Case_Open");
    if(nStatus & 0x02) strcat(buffer, " Tamper");
    if(nStatus & 0x04) strcat(buffer, " Vibration");
    if(nStatus & 0x08) strcat(buffer, " Unstable(Shut-Off)");
    if(nStatus & 0x10) strcat(buffer, " Bad_Op(Open)");
    if(nStatus & 0x20) strcat(buffer, " Bad_Op(Shut-Off)");
    strcat(buffer," )");
}

const char *MSGAPI_GetKdMeterStatus(BYTE nStatus)
{
	switch(nStatus) {
	  case 1 :	return "Open";
	  case 2 :	return "Closed (Manual)";
	  case 3 :	return "Standby";
	  case 4 :	return "Closed (Vibration)";
	  case 5 :	return "Closed (Leak)";
	  case 6 :	return "Closed (Remote)";
	}
	return "Unknown";
}

const char *MSGAPI_GetKdValveState(BYTE nState)
{
	switch(nState) {
	  case 0 :	return "open";
	  case 1 :	return "close";
