...
int check_hex_string(char *pszString, int len)
{
    ...
    if(strlen(pszString) > sizeof(strBuffer)-1)
    {
        return 0;
    }

    strcpy(strBuffer,pszString);
    for(str=strBuffer; ; str = NULL)
    {
...
int check_opvalve(CLISESSION *pSession, char *pszValue)
{
	...
	n = strtol(pszValue, (char **)NULL, 10);
	if (strcasecmp(pszValue, "open") == 0)
		n = 0;
	else if (strcasecmp(pszValue, "close") == 0)
		n = 1;
	else if (strcasecmp(pszValue, "standby") == 0)
		n = 2;
	else if (n > 2 || n < 0)
		return 0;
