...
int cmdOndemandRelay(CLISESSION *pSession, int argc, char **argv, void *pHandler)
{
	...
	{
		pTable = (GETABLE *)invoke.GetHandle()->pResult[0]->stream.p;
		OUTTEXT(pSession, "\r\n");
		OUTTEXT(pSession, "     Relay status : %s\r\n", (pTable->data[0] & 0xff) == 1 ? "on" : "off");
		OUTTEXT(pSession, "Activation status : %s\r\n", (pTable->data[1] & 0xff) == 1 ? "on" : "off");
	}
	return CLIERR_OK;
}
#endif
...
int cmdOndemandCid(CLISESSION *pSession, int argc, char **argv, void *pHandler)
{
    ...
    else
    {
        ...
        else if(!strncasecmp(argv[1], "GetSt", 5))      // GetStatusLog
        {
            ...
            memcpy(_common_arr, _statuslog_arr, sizeof(_statuslog_arr)); 
            szStrParam = &_statuslog_arr[0];
            nParamLen = sizeof(_statuslog_arr) + 2;
        ...
        else if(!strncasecmp(argv[1], "GetRt", 5))      // GetRtcLog
        {
            nCid = KMP_CID_GET_LOG_ID_PRESENT;
            memset(_common_arr, 0, sizeof(_common_arr));
            memcpy(_common_arr, _rtclog_arr, sizeof(_rtclog_arr)); 
            szStrParam = &_rtclog_arr[0];
            nParamLen = sizeof(_rtclog_arr) + 2;
        ...
        else if(!strncasecmp(argv[1], "GetV", 4))       // GetVQualityLog
        {
            nCid = KMP_CID_GET_LOG_ID_PRESENT;
            memset(_common_arr, 0, sizeof(_common_arr));
            memcpy(_common_arr, _vqlog_arr, sizeof(_vqlog_arr)); 
            szStrParam = &_vqlog_arr[0];
            nParamLen = sizeof(_vqlog_arr) + 2;
            m_nKmpCommand = KMP_CMD_VQLOG;
        }
        else if(!strncasecmp(argv[1], "GetCu", 5))      // GetCutOffLog
        {
            nCid = KMP_CID_GET_LOG_ID_PRESENT;
            memset(_common_arr, 0, sizeof(_cutofflog_arr));
            memcpy(_common_arr, _cutofflog_arr, sizeof(_cutofflog_arr)); 
            szStrParam = &_cutofflog_arr[0];
            nParamLen = sizeof(_cutofflog_arr) + 2;
    ...
    kamstrup_data_parsing(pSession, METER_VENDOR_KAMSTRUP, 
            invoke.GetHandle()->pResult[0]->stream.p, invoke.GetHandle()->pResult[0]->len, FALSE);
...
{
    CIF4Invoke  invoke("127.0.0.1", m_nLocalAgentPort);
    BYTE        szProperty[1024];
    int         nMode;
    char        *p;
    int         n=0, propertyLen=0, strLen, idx;

    memset(szProperty, 0, sizeof(szProperty));
    invoke.AddParam("1.11", argv[0]);
    invoke.AddParam("1.11", argv[1]);
    invoke.AddParam("1.11", argv[2]);
    invoke.AddParam("1.11", argv[3]);

    nMode = (strcasecmp(argv[4], "A") == 0) ? 0 : 1;
    if (nMode == 0)
    {
        // ASCII Mode
        if (strlen(argv[5]) > sizeof(szProperty))
        {
            OUTTEXT(pSession, "Invalid length (max=%d).\r\n", sizeof(szProperty));
            return CLIERR_OK;
        }
        propertyLen = strlen(argv[5]);
        memcpy(szProperty, argv[5], propertyLen);
    }
    else
    {
        strLen = strlen(argv[5]);
        // HEX Mode
        for(n=0, p=argv[5], idx=0; p && *p && idx < strLen; p++, idx+=3)
        {
            if (((n == 0) && (*p == ' ')) ||
                ((n == 1) && (*p == ' ')) ||
                ((n == 2) && (*p != ' ')))
            {
                   OUTTEXT(pSession, "Invalid HEX format. ex> H \"4D 41 53 54 45 52 5F 30 31 32\"\r\n");
                   return CLIERR_OK;
            }
            n++;
            if (n > 2) n = 0;
        ...
        for(p=argv[5], idx=0; p && *p && idx < strLen; p+=3, propertyLen++, idx+=3)
        {
            if(propertyLen >= (int)sizeof(szProperty))
            {
                OUTTEXT(pSession, "Invalid length (max=%d).\r\n", sizeof(szProperty));
                return CLIERR_OK;
            }
            sscanf(p, "%x", &n);
            szProperty[propertyLen] = n;
        }
    }
    ...
    invoke.AddParam("1.12", szProperty, propertyLen);
    if (!CLIAPI_Command(pSession, invoke.GetHandle(), "104.21"))
        return CLIERR_OK;
...
int cmdAddMeterSecurity(CLISESSION *pSession, int argc, char **argv, void *pHandler)
{
    ...
    else
    {
        ...
        // HEX Mode
        for(n=0, p=argv[3], idx=0; p && *p && idx < strLen; p++, idx+=3)
        {
            if (((n == 0) && (*p == ' ')) ||
                ((n == 1) && (*p == ' ')) ||
                ((n == 2) && (*p != ' ')))
        ...
        for(p=argv[3], idx=0; p && *p && idx < strLen; p+=3, secLen++, idx+=3)
        {
...
{
    CIF4Invoke  invoke("127.0.0.1", m_nLocalAgentPort);
    int         nOption = -1, nOffset = -1, nCount = -1;
    ...
    nOption = (int)strtol(argv[2], (char **)NULL, 10);
    nOffset = (int)strtol(argv[3], (char **)NULL, 10);
    nCount = (int)strtol(argv[4], (char **)NULL, 10);

    invoke.AddParam("1.9", nOption);
    invoke.AddParam("1.9", nOffset);
    invoke.AddParam("1.9", nCount);

    if (!CLIAPI_Command(pSession, invoke.GetHandle(), "104.19"))
        return CLIERR_OK;
 ...
 *
 *	SYNTAX :
 *		ondemand valve <eui64id> <valve status>
 *
 *	DESCRIPTION :
 *		지정된 센서와 연결된 미터의 벨브를 열거나 차단한다.
 *
 *	COMMAND :
 *		cmdOndemandValve(105.9)
 *				4.2.3		EUI64		EUI64 ID
 *				1.4			BYTE		Operation Code
 *										0 : Open
 *										1 : Close
 *										2 : Standby (미터를 수동으로 조작 가능)
 *				
 *--------------------------------------------------------------------------------*/
int cmdOndemandValve(CLISESSION *pSession, int argc, char **argv, void *pHandler)
{
    ...
    if (strcasecmp(argv[1], "open") == 0)
	    ...
	    opcode = 1;
	else if (strcasecmp(argv[1], "standby") == 0)
	    opcode = 2;
	else opcode = atoi(argv[1]);

	if (!Confirm(pSession, MSG_DEFAULT))
		return CLIERR_OK;

  	invoke.AddParamFormat("4.2.3", VARSMI_EUI64, (void *)&id, sizeof(EUI64));
	invoke.AddParam("1.4", opcode);
	if (!CLIAPI_Command(pSession, invoke.GetHandle(), "105.11"))
		return CLIERR_OK;

    OUTTEXT(pSession, "The valve status of meter is changed to %s successfully.\r\n", MSGAPI_GetKdValveState(opcode));
	return CLIERR_OK;
}
#endif
