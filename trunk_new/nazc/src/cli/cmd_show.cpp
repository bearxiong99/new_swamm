...
int cmdShowSystem(CLISESSION *pSession, int argc, char **argv, void *pHandler)
{
	...
	double		fHwVer, fSwVer;
	int			nType;
#if defined(__DISABLE_SSL__)
	BYTE		nState, *addr;
#else
	BYTE		nState, *addr, nServerSecurity = 0;
#endif
	...
	invoke2.AddParam("2.12");
	if (!CLIAPI_Command(pSession, invoke2.GetHandle(), "199.4"))
		return CLIERR_OK;
	...
	OUTTEXT(pSession, "MOBILE TYPE : %-25s   MODE : %s(%0d)\xd\xa",
                        invoke2.ResultAtName("moduleType")->stream.p,
...
/** GPS와 timezone 정보는 뺀다
	OUTTEXT(pSession, "     GPS timesync : %-10s           Timezone : GMT%s%d\xd\xa",
									...
									tzone);
*/
    OUTTEXT(pSession, "         Topology : %-18s  Join Ctrl : %s\xd\xa", MSGAPI_TimeSyncStrategy(pValue->varTimeSyncStrategy),
		                            pEnable->varEnableJoinControl ? "Enable" : "Disable");
	...
	OUTTEXT(pSession, "    Fail Recovery : %-9s \xd\xa", // Issue #855로 Metering Option을 지정하기 때문에 Default Option 항목을 제외한다
	    ...
	    for(i=0; i<nCount; i++)
	    {
            ...
            switch(parserType) {
                ...
                case PARSER_TYPE_I210:
                    pulse_data_parsing(pSession, pSensor->sensorModel, pItem->mlpData, 
                            LittleToHostShort(pItem->mlpLength) - sizeof(TIMESTAMP), bDetail, EXTRA_NONE);
                    break;
                case PARSER_TYPE_KEUKDONG:
                    pulse_data_parsing(pSession, pSensor->sensorModel, pItem->mlpData, 
                            LittleToHostShort(pItem->mlpLength) - sizeof(TIMESTAMP), bDetail, EXTRA_KEUKDONG);
                    break;
                case PARSER_TYPE_KAMST:
                case PARSER_TYPE_KAMST_NEW:
                    kamstrup_data_parsing(pSession, pItem->mlpVendor, pItem->mlpData, 
                            LittleToHostShort(pItem->mlpLength) - sizeof(TIMESTAMP), TRUE);
                    break;
...
int cmdShowMobile(CLISESSION *pSession, int argc, char **argv, void *pHandler)
{
	...
	invoke.AddParam("2.12");
	if (!CLIAPI_Command(pSession, invoke.GetHandle(), "199.4"))
		return CLIERR_OK;

    //OUTTEXT(pSession, " MobileType : %s\xd\xa", invoke.ResultAtName("moduleType")->stream.p);
    OUTTEXT(pSession, "Interface : %s (Baud %d Flag %04X)\xd\xa", invoke.ResultAtName("interface")->stream.p,
                invoke.ResultAtName("interfaceSpeed")->stream.u32,
                invoke.ResultAtName("moduleFlag")->stream.u16);
    if(strlen(invoke.ResultAtName("sms")->stream.p) > 0)
    {
        OUTTEXT(pSession, "      SMS : %s (%d)\xd\xa", invoke.ResultAtName("sms")->stream.p, 
                invoke.ResultAtName("smsSpeed")->stream.u32);
    }
	...
	if (fp != NULL)
	{
		while(fgets(szBuffer, 127, fp))
		{
            ...
            } else if (strncmp(szBuffer, "MANF ", 5) == 0) {
                strcpy(szVendor, &szBuffer[5]);
            } else if (strncmp(szBuffer, "MODEL ", 6) == 0) {
                strcpy(szModel, &szBuffer[6]);
            } else if (strncmp(szBuffer, "IMSI ", 5) == 0) {
                strcpy(szSCID, &szBuffer[5]);
            } else if (strncmp(szBuffer, "CSQ ", 4) == 0) {
				strcpy(szCSQ, &szBuffer[4]);
                nCSQ = (int)strtol(szCSQ, (char**)NULL, 10);
				if (nCSQ<99)
                {
					 nCSQ = -(113-(nCSQ*2));
                }
                else nCSQ = MIN(0, nCSQ);
            }
...
int _cmdShowLogFile(CLISESSION *pSession, int argc, char **argv, void *pHandler, char * logFile)
{
    char    szPath[64];
    char    szBuffer[256];
    time_t  now;
    ...
    if (argc == 3)
         sprintf(szPath, "%s/%s%04d%02d%02d.log", LOG_DIR, logFile,
...
{
    return _cmdShowLogFile(pSession, argc, argv, pHandler, const_cast<char*>(CMD_HISTORY_LOG_FILE));
}

int cmdShowCommLog(CLISESSION *pSession, int argc, char **argv, void *pHandler)
{
    return _cmdShowLogFile(pSession, argc, argv, pHandler, const_cast<char*>(COMM_LOG_FILE));
}

int cmdShowMobileLog(CLISESSION *pSession, int argc, char **argv, void *pHandler)
{
    return _cmdShowLogFile(pSession, argc, argv, pHandler, const_cast<char*>(MOBILE_LOG_FILE));
}

int cmdShowUploadLog(CLISESSION *pSession, int argc, char **argv, void *pHandler)
{
    return _cmdShowLogFile(pSession, argc, argv, pHandler, const_cast<char*>(UPLOAD_LOG_FILE));
}

int cmdShowTimesyncLog(CLISESSION *pSession, int argc, char **argv, void *pHandler)
{
    return _cmdShowLogFile(pSession, argc, argv, pHandler, const_cast<char*>(TIMESYNC_LOG_FILE));
}

int cmdShowMeteringLog(CLISESSION *pSession, int argc, char **argv, void *pHandler)
{
    return _cmdShowLogFile(pSession, argc, argv, pHandler, const_cast<char*>(METERING_LOG_FILE));
}

int cmdShowEvent(CLISESSION *pSession, int argc, char **argv, void *pHandler)
{
    return _cmdShowLogFile(pSession, argc, argv, pHandler, const_cast<char*>(EVENT_LOG_FILE));
}
...
{
    return _cmdShowLogFile(pSession, argc, argv, pHandler, const_cast<char*>(UPGRADE_LOG_FILE));
}
...
void _displayProperty(CLISESSION *pSession, IF4Invoke *pInvoke, int nListSize)
{
    char        *pGroup=NULL, *pProperty=NULL, *pType=NULL, *pKey=NULL;
    int         i, j, nCount;

	nCount  = pInvoke->nResultCount;

    OUTTEXT(pSession, "==========================================================================================\r\n");
    OUTTEXT(pSession, " NUM GROUP              PROPERTY           TYPE   KEY                VALUE\r\n");
    OUTTEXT(pSession, "==========================================================================================\r\n");

	for (i=0; i<= (nCount-nListSize); i+=nListSize)
	{
        OUTTEXT(pSession, " %3d %-18s %-18s %-6s %-18s ", (i/nListSize)+1, 
                (pGroup != NULL && !strcmp(pGroup, (char *)pInvoke->pResult[i]->stream.p) ? 
                    "" : (char *)pInvoke->pResult[i]->stream.p), 
                (pProperty != NULL && !strcmp(pProperty, (char *)pInvoke->pResult[i+1]->stream.p) ? 
                    "" : (char *)pInvoke->pResult[i+1]->stream.p), 
                (pType != NULL && !strcmp(pType, (char *)pInvoke->pResult[i+2]->stream.p) && 
                 pGroup != NULL && !strcmp(pGroup, (char *)pInvoke->pResult[i]->stream.p) ? 
                    "" : (char *)pInvoke->pResult[i+2]->stream.p), 
                (pKey != NULL && !strcmp(pKey, (char *)pInvoke->pResult[i+3]->stream.p) && 
                 pGroup != NULL && !strcmp(pGroup, (char *)pInvoke->pResult[i]->stream.p) ? 
                    "" : (char *)pInvoke->pResult[i+3]->stream.p));

        for(j=0; j<pInvoke->pResult[i+4]->len;j++)
        {
            OUTTEXT(pSession, "%02X ", pInvoke->pResult[i+4]->stream.p[j]);
        }
        OUTTEXT(pSession, "\r\n");
        pGroup = (char *)pInvoke->pResult[i]->stream.p;
        pProperty = (char *)pInvoke->pResult[i+1]->stream.p;
        pType = (char *)pInvoke->pResult[i+2]->stream.p;
        pKey = (char *)pInvoke->pResult[i+3]->stream.p;
	}
    OUTTEXT(pSession, "==========================================================================================\r\n");

}

int cmdShowMeterProperty(CLISESSION *pSession, int argc, char **argv, void *pHandler)
{
	CIF4Invoke	invoke("127.0.0.1", m_nLocalAgentPort);
	IF4Invoke   *pInvoke;
    int         idx=0, listSize=5;

    if(argc>0)
    {
        invoke.AddParamFormat("1.11", VARSMI_STRING, argv[idx], strlen(argv[idx])); // group
        idx ++;
    }
    if(argc>1)
    {
        invoke.AddParamFormat("1.11", VARSMI_STRING, argv[idx], strlen(argv[idx])); // property
        idx ++;
    }
    if(argc>2)
    {
        invoke.AddParamFormat("1.11", VARSMI_STRING, argv[idx], strlen(argv[idx])); // type
        idx ++;
    }
    if(argc>3)
    {
        invoke.AddParamFormat("1.11", VARSMI_STRING, argv[idx], strlen(argv[idx])); // key
        idx ++;
    }

	pInvoke = (IF4Invoke *)invoke.GetHandle();
	if (!CLIAPI_Command(pSession, pInvoke, "104.20"))
		return CLIERR_OK;

    _displayProperty(pSession, pInvoke, listSize);

    return CLIERR_OK;
}

int cmdShowMeterPropertyById(CLISESSION *pSession, int argc, char **argv, void *pHandler)
{
	CIF4Invoke	invoke("127.0.0.1", m_nLocalAgentPort);
	IF4Invoke   *pInvoke;
    int         idx=0, listSize=5;
	...
	pInvoke = (IF4Invoke *)invoke.GetHandle();
	if (!CLIAPI_Command(pSession, pInvoke, "104.22"))
		return CLIERR_OK;
...
int cmdShowMeterSecurity(CLISESSION *pSession, int argc, char **argv, void *pHandler)
{
    ...
    OUTTEXT(pSession, "================================================================================\r\n");
    OUTTEXT(pSession, " NUM TYPE     KEY                VALUE \r\n");
    OUTTEXT(pSession, "================================================================================\r\n");
...
int cmdShowMeteringOption(CLISESSION *pSession, int argc, char **argv, void *pHandler)
{
	CIF4Invoke	invoke("127.0.0.1", m_nLocalAgentPort);
	IF4Invoke   *pInvoke;
    char        *pType = NULL, *pName = NULL;
	...
	pInvoke = (IF4Invoke *)invoke.GetHandle();
	if (!CLIAPI_Command(pSession, pInvoke, "104.18"))
		return CLIERR_OK;

	nCount  = pInvoke->nResultCount;

    OUTTEXT(pSession, "============================================================\r\n");
    OUTTEXT(pSession, " NUM TYPE     KEY                OPTION OFFSET  COUNT \r\n");
    OUTTEXT(pSession, "============================================================\r\n");

	for (i=0; i<= (nCount-5); i+=5)
	{
        OUTTEXT(pSession, " %3d %-8s %-18s %6d %6d %6d\r\n", (i/3)+1, 
                pType != NULL && !strcmp(pType, (char *)pInvoke->pResult[i]->stream.p) ? 
                    "" : (char *)pInvoke->pResult[i]->stream.p, 
                pName != NULL && !strcmp(pName, (char *)pInvoke->pResult[i+1]->stream.p) ? 
                    "" : (char *)pInvoke->pResult[i+1]->stream.p,
                    pInvoke->pResult[i+2]->stream.s32,
                    pInvoke->pResult[i+3]->stream.s32,
                    pInvoke->pResult[i+4]->stream.s32);
...
int cmdShowKdMeterStatus(CLISESSION *pSession, int argc, char **argv, void *pHandler)
{
    CIF4Invoke  invoke("127.0.0.1", m_nLocalAgentPort, EXTENDED_TIMEOUT);
	IF4Invoke	*pInvoke;
	char		szGUID[MAX_GUID_STRING+1];
	...
	StrToEUI64(argv[0], &id);
    invoke.AddParamFormat("4.2.3", VARSMI_EUI64, &id, sizeof(EUI64));
	if (!CLIAPI_Command(pSession, invoke.GetHandle(), "105.12"))
		return CLIERR_OK;

	pInvoke = (IF4Invoke *)invoke.GetHandle();
	EUI64ToStr(&id, szGUID);

	if (pInvoke->nResultCount >= 4)
	{
		OUTTEXT(pSession, "     SENSOR ID : %s\xd\xa", szGUID);
		OUTTEXT(pSession, "     METER S/N : %s\xd\xa", pInvoke->pResult[1]->stream.p ? pInvoke->pResult[1]->stream.p : "");
		OUTTEXT(pSession, " CURRENT PULSE : %d\xd\xa", pInvoke->pResult[0]->stream.u32);
		OUTTEXT(pSession, "  METER STATUS : 0x%02X %s\xd\xa", pInvoke->pResult[3]->stream.u8, 
											MSGAPI_GetKdMeterStatus(pInvoke->pResult[3]->stream.u8));
   		OUTTEXT(pSession, "  ALARM STATUS : 0x%02X\xd\xa", pInvoke->pResult[2]->stream.u8);
		if (pInvoke->pResult[2]->stream.u8 & 0x01) OUTTEXT(pSession, "                 Case open\xd\xa");
		if (pInvoke->pResult[2]->stream.u8 & 0x02) OUTTEXT(pSession, "                 Tamper\xd\xa");
		if (pInvoke->pResult[2]->stream.u8 & 0x04) OUTTEXT(pSession, "                 Vibration\xd\xa");
		if (pInvoke->pResult[2]->stream.u8 & 0x08) OUTTEXT(pSession, "                 Close unstable\xd\xa");
		if (pInvoke->pResult[2]->stream.u8 & 0x10) OUTTEXT(pSession, "                 Close fail\xd\xa");
		if (pInvoke->pResult[2]->stream.u8 & 0x20) OUTTEXT(pSession, "                 Open fail\xd\xa");
	}
  	return CLIERR_OK;
}

int cmdShowKdMeterVersion(CLISESSION *pSession, int argc, char **argv, void *pHandler)
{
    CIF4Invoke  invoke("127.0.0.1", m_nLocalAgentPort, EXTENDED_TIMEOUT);
	IF4Invoke	*pInvoke;
	char		szGUID[MAX_GUID_STRING+1];
	...
	StrToEUI64(argv[0], &id);
    invoke.AddParamFormat("4.2.3", VARSMI_EUI64, &id, sizeof(EUI64));
	if (!CLIAPI_Command(pSession, invoke.GetHandle(), "105.13"))
		return CLIERR_OK;

	pInvoke = (IF4Invoke *)invoke.GetHandle();
	EUI64ToStr(&id, szGUID);

	if (pInvoke->nResultCount >= 3)
	{
		if (pInvoke->pResult[0]->stream.p)
		{
			memset(szDate, 0, sizeof(szDate));
			strncpy(szDate, pInvoke->pResult[0]->stream.p, 6);
			flagAccept = pInvoke->pResult[0]->stream.p[6];
		}

		OUTTEXT(pSession, "     SENSOR ID : %s\xd\xa", szGUID);
		OUTTEXT(pSession, "   S/W Version : %s\xd\xa", pInvoke->pResult[1]->stream.p ? pInvoke->pResult[1]->stream.p : "");
   	 	OUTTEXT(pSession, "   H/W Version : %s\xd\xa", pInvoke->pResult[2]->stream.p ? pInvoke->pResult[2]->stream.p : "");
		OUTTEXT(pSession, "     Test DATE : %s\xd\xa", szDate);
		OUTTEXT(pSession, "   Test Result : %c %s\xd\xa", flagAccept, (flagAccept == 'A') ? "(Accept)" : "");
	}
  	return CLIERR_OK;
}
#endif
