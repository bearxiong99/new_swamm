...
{    
    ...
    ioctl(fd, GPIO_IOCTL_DIR, GPIO11_OUT_SET);      // MOBILE POWER (LOW - HIGH)
    ioctl(fd, GPIO_IOCTL_DIR, GPIO30_OUT_SET);      // MOBILE RESET (IGT) (HIGH - LOW - HIGH)
    ioctl(fd, GPIO_IOCTL_DIR, GPIO40_OUT_SET);      // MOBILE DTR (HIGH - LOW)
    ioctl(fd, GPIO_IOCTL_DIR, GPIO84_OUT_SET);      // MOBILE EMERGY OFF (HIGH - LOW)
    
    // GSM Mobule Power Control
    write(0, "INIT: Mobile Power ON          \r", 32);
    ioctl(fd, GPIO_IOCTL_OUT, GPIO11_OUT_HIGH);
    
    write(0, "INIT: Mobile Reset.            \r", 32);
    ioctl(fd, GPIO_IOCTL_OUT, GPIO30_OUT_HIGH);
    usleep(1000000);
    ioctl(fd, GPIO_IOCTL_OUT, GPIO30_OUT_LOW);
    usleep(1000000);
    ioctl(fd, GPIO_IOCTL_OUT, GPIO30_OUT_HIGH);
    
    // GSM Module Disconnect
    write(0, "INIT: Mobile DTR Control.      \r", 32);
    ioctl(fd, GPIO_IOCTL_OUT, GPIO40_OUT_HIGH);
    usleep(1000000);
    ioctl(fd, GPIO_IOCTL_OUT, GPIO40_OUT_LOW);
...
{    
    ...
    ioctl(fd, GPIO_IOCTL_DIR, GPIO40_OUT_SET);
    ioctl(fd, GPIO_IOCTL_OUT, GPIO40_OUT_HIGH);
    usleep(1000000);
    ioctl(fd, GPIO_IOCTL_OUT, GPIO40_OUT_LOW);
	...
	for(nState=STATE_INIT; (!m_bExitSignalPending) && (nState!=STATE_DONE);)
	{
		switch(nState) {
		  ...
		  case STATE_SELECT :
			   ...
			   usleep(1000000);
			   pClient->WriteToModem("NURI/1.0 CMD Reset\n\n");
			   return 0;
		}
	}
	return -3;
