...
void if4event()
{
    ...
    SendEvent(const_cast<char *>("240.240"),&invoke);
...
int if4call(char * ipaddr, int port)
{
    IF4Invoke * pInvoke = NULL;
    int         nError;
	char		meterSerial[12] = "08140008599";//"08140008620";

    CIF4Invoke  invoke(ipaddr, port, 15);

    /*
    invoke.AddParamFormat("1.16", VARSMI_TIMESTAMP, &tmEvent, sizeof(TIMESTAMP));
    */

	invoke.AddParamFormat("1.11", VARSMI_STRING, &meterSerial, sizeof(meterSerial));

    pInvoke = (IF4Invoke *)invoke.GetHandle();
    //nError = IF4API_Command(pInvoke, "211.211", IF4_CMDATTR_REQUEST | IF4_CMDATTR_RESPONSE);
	nError = IF4API_Command(pInvoke, "225.1", IF4_CMDATTR_REQUEST | IF4_CMDATTR_RESPONSE);

    if(nError != IF4ERR_NOERROR) return -nError;

    XDEBUG("nResultCount=%d\n", pInvoke->nResultCount); 
    /*
    for(i=0;i< pInvoke->nResultCount; i++)
    {
        memcpy(&tmEvent, pInvoke->pResult[i]->stream.p, sizeof(TIMESTAMP));
