...
void CTransfer::IgtControl()
{
	...
	XDEBUG("CIRCUIT: Modem IGT/DTR Control.\r\n");
    fd = open("/dev/gpio", O_RDWR | O_NDELAY);
    if (fd <= 0)
        return; 
...
int CTransfer::GetBatteryState()
{
    ...
    if (fd > 0)
  	{ 
	    // GPIO Input Direction Setting
		ioctl(fd, GPIO_IOCTL_DIR, GP_BATT_CHG_STATUS_INPUT);
		batt = ioctl(fd, GPIO_IOCTL_IN, GP_BATT_CHG_STATUS_INPUT);
		XDEBUG("BATT CARGE SATE = %d\r\n", batt);
		close(fd);
	}
	return batt;
#else
